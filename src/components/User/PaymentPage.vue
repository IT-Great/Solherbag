...
<!-- <template>
  ...
  <div v-else class="max-w-6xl min-h-screen px-6 py-12 mx-auto md:py-24 animate-fade-in">
            ...
            <div class="space-y-4">
              ...
              >
                <div class="grid grid-cols-1 gap-6 p-4 md:grid-cols-2 bg-gray-50 rounded-2xl">
                  <div>
                    <h3 class="font-bold text-[10px] text-gray-400 uppercase tracking-[0.2em] mb-3">
                      {{ $t("payment.destination") }}
                    </h3>
                    <p class="text-xs font-bold text-gray-900 uppercase">
                      {{ destinationInfo?.name }}
                    ...
                    <p class="mt-1 text-xs text-gray-500 line-clamp-2">
                      {{ destinationInfo?.address }} - {{ destinationInfo?.postal_code }}
                    </p>
                    <p class="mt-1 text-xs font-bold text-blue-600 uppercase">
                      {{ destinationInfo?.country }}
                    </p>
                ...
                <div v-if="destinationInfo?.country === 'Indonesia'">
                  ...
                  <div class="flex flex-col gap-4 mb-4 md:flex-row">
                    <label
                      :class="deliveryType === 'now' ? 'border-black bg-gray-50' : 'border-gray-200'"
                      class="flex-1 p-4 transition border cursor-pointer rounded-xl"
                    >
                    ...
                    <label
                      :class="deliveryType === 'scheduled' ? 'border-black bg-gray-50' : 'border-gray-200'"
                      class="flex-1 p-4 transition border cursor-pointer rounded-xl"
                    >
                ...
                <div>
                  <h3 class="pt-6 mb-4 text-sm font-bold tracking-widest uppercase border-t">
                    {{ $t("payment.select_courier") }}
                  </h3>
                      ...
                      <div class="flex items-center w-full">
                        ...
                        <div class="flex items-center flex-grow gap-4 ml-4">
                          <div class="flex items-center justify-center w-12 h-12 overflow-hidden bg-white border border-gray-100 rounded-lg shrink-0">
                            ...
                            />
                            <span
                              v-show="imageErrors[rate.company] || !getCourierLogo(rate.company)"
                              class="text-xs font-black text-gray-300"
                            >
                              {{ rate.company.toUpperCase() }}
                            </span>
          ...
          <div class="sticky p-8 bg-white border border-gray-100 shadow-xl rounded-3xl top-28">
            ...
            <div class="space-y-4 mb-8 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              <div v-for="item in checkoutItems" :key="item.id" class="flex gap-4">
                ...
                <div class="flex-grow">
                  <div class="flex items-center gap-2">
                    <p class="w-40 text-[11px] font-bold text-gray-900 uppercase truncate" :title="item.product?.name">
                      {{ item.product?.name }}
                    </p>
                    <span
                      v-if="userType === 'reseller' && Number(item.product?.wholesale_price) > 0 && checkoutCount >= 24"
                      class="px-1.5 py-0.5 text-[8px] font-bold text-white bg-blue-600 rounded"
                    >GROSIR</span>
                  </div>
                  <div class="flex items-center gap-2 mt-0.5">
            ...
            <div class="pt-4 space-y-3 text-sm border-t border-gray-50">
              ...
              <div class="flex justify-between text-gray-500">
                <span>{{ $t("payment.subtotal") }}</span>
                <span :class="appliedPromoType === 'voucher' ? 'text-amber-600 font-bold' : ''">
                  {{ formatCurrencyDisplay(cartSubtotalObj) }}
                </span>
              </div>

              <div v-if="userData?.is_membership" class="pt-4 mt-2 border-t border-gray-200 border-dashed">
                <div class="flex items-center justify-between">
                  <div>
                    <p class="text-[10px] font-bold text-gray-900 uppercase tracking-widest flex items-center gap-1">
                    ...
                    <p v-if="checkoutTotalIDR < MEMBER_MIN_SPEND" class="text-[8px] text-red-500 italic mt-0.5">
                      {{ $t("payment.min_spend") }}
                    </p>
                  </div>
                  <label class="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      @change="handleMemberToggle"
                      class="sr-only peer"
                      :disabled="isVerifyingPromo || (checkoutTotalIDR < MEMBER_MIN_SPEND && !useMemberVoucher)"
                    />
              ...
              <div class="pt-4 mt-2 border-t border-gray-200 border-dashed">
                <label class="text-[10px] font-bold text-gray-900 uppercase tracking-widest mb-2 block">
                  {{ $t("payment.promo_code") }}
                </label>
                <form @submit.prevent="applyPromo" class="flex gap-2">
                  <input
                    type="text"
                    v-model="promoInput"
                    :disabled="appliedPromoCode !== null || isVerifyingPromo || useMemberVoucher"
                    :placeholder="$t('payment.enter_promo_code')"
                    class="flex-1 bg-white border border-gray-300 rounded-lg px-3 py-1.5 text-sm uppercase focus:ring-black outline-none disabled:bg-gray-100 disabled:text-gray-400 transition-colors"
                  />
                  <button
                ...
                <p v-if="promoMessage" :class="promoSuccess ? 'text-green-600' : 'text-red-500'" class="text-[10px] mt-2 font-medium">
                  {{ promoMessage }}
                </p>
              ...
              <div v-if="userData?.is_membership && userData?.point > 0" class="pt-4 mt-2 border-t border-gray-200 border-dashed">
                <div class="flex items-center justify-between mb-2">
                  ...
                  <span class="text-xs text-gray-500">{{ $t("payment.bal") }} {{ userData.point }} Pts</span>
                </div>
                <div class="flex gap-2">
                  <input type="number" v-model="pointsToUse" :max="maxUsablePoints" min="0" class="flex-1 bg-white border border-yellow-300 rounded-lg px-3 py-1.5 text-sm focus:ring-yellow-500 outline-none" placeholder="0" />
                  <button @click="useAllPoints" class="bg-yellow-100 text-yellow-800 text-[10px] font-bold uppercase px-3 rounded-lg hover:bg-yellow-200 transition">
                    {{ $t("payment.use_all") }}
                  </button>
                </div>
                <p v-if="pointsToUse > 0" class="text-[10px] text-green-600 mt-1 font-medium text-right">
              ...
              <div class="flex items-start justify-between text-gray-500">
                ...
                <div v-else-if="shippingMethod === 'biteship' && selectedRate" class="text-right">
                  <span class="block font-medium text-gray-900">
                    {{ formatCurrencyDisplay({ value: shippingCostObj.value * checkoutCount, curr: shippingCostObj.curr }) }}
                  </span>
                  <p class="mt-1 text-[10px] text-gray-400">
                    {{ formatCurrencyDisplay(shippingCostObj) }} x {{ checkoutCount }} {{ $t("payment.item") }}
                  </p>
                </div>
              ...
              <p v-if="!selectedAddressId" class="mt-4 text-[10px] tracking-tighter text-center text-red-500 uppercase">
                {{ $t("payment.select_shipping_address") }}
              </p>
              <p v-else-if="shippingMethod === 'biteship' && !selectedRate" class="mt-4 text-[10px] tracking-tighter text-center text-red-500 uppercase">
                {{ $t("payment.select_courier_service") }}
              </p>
            </div>
          </div>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 pt-10 pb-10 overflow-y-auto bg-black/60 backdrop-blur-sm animate-fade-in-up">
      <div class="flex flex-col w-full max-w-5xl my-auto overflow-hidden bg-white shadow-2xl rounded-3xl">
        <div class="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50 shrink-0 md:p-8 md:pb-6">
          <h3 class="text-xl font-extrabold tracking-tight text-gray-900">
            {{ editingId ? $t("modal_edit_address_title") : $t("modal_add_address_title") }}
          </h3>
          <button @click="isModalOpen = false" class="p-2 text-gray-400 transition-colors bg-white border border-gray-200 rounded-full hover:text-gray-900 hover:bg-gray-100 focus:outline-none">✕</button>
        </div>

        <div class="flex flex-col md:flex-row h-auto md:h-[650px] overflow-hidden">
          <div class="relative w-full bg-gray-100 border-b border-gray-200 h-72 md:h-full md:w-5/12 md:border-b-0 md:border-r shrink-0">
            <l-map ref="map" v-model:zoom="zoom" :center="center" :use-global-leaflet="false" @click="onMapClick" style="height: 100%; width: 100%;">
              <l-tile-layer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" layer-type="base" name="OpenStreetMap"></l-tile-layer>
              <l-marker :lat-lng="markerLatLng" draggable @update:latLng="onMarkerDrag"></l-marker>
            </l-map>

            <button type="button" @click="getCurrentLocation" :disabled="isGettingLocation" class="absolute z-[1000] bottom-6 left-1/2 -translate-x-1/2 md:left-auto md:-translate-x-0 md:right-6 flex items-center gap-2 bg-white text-gray-900 px-5 py-2.5 rounded-full shadow-xl font-bold text-xs hover:bg-gray-50 border border-gray-200 transition-all hover:-translate-y-0.5">
              <span v-if="isGettingLocation" class="w-4 h-4 border-2 rounded-full border-[#006A4E] border-t-transparent animate-spin"></span>
              <svg v-else class="w-4 h-4 text-[#006A4E]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
              {{ $t("payment.current_loc") }}
            </button>
          </div>

          <form @submit.prevent="saveAddress" class="flex flex-col flex-1 p-6 space-y-6 overflow-y-auto bg-white sm:p-8 custom-scrollbar">
            <div class="flex gap-3 p-4 border border-blue-100 rounded-2xl bg-blue-50/50">
              ...
              <p class="text-xs leading-relaxed text-blue-800">{{ $t("guide_map_text") }}</p>
            </div>

            <div class="space-y-5">
              <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div>
                  <label class="block mb-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ $t("label_first_name") }}</label>
                  <input type="text" required v-model="form.first_name_address" class="w-full p-3 text-sm font-medium border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#006A4E] outline-none transition-all" />
                </div>
                <div>
                  <label class="block mb-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ $t("label_last_name") }}</label>
                  <input type="text" required v-model="form.last_name_address" class="w-full p-3 text-sm font-medium border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#006A4E] outline-none transition-all" />
                </div>
              </div>

              <div>
                <label class="block mb-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ $t("label_address_type") }}</label>
                <select v-model="form.location_type" class="w-full p-3 text-sm font-medium border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#006A4E] outline-none bg-white transition-all">
                  <option value="home">{{ $t("option_home") }}</option>
                  <option value="office">{{ $t("option_office") }}</option>
                  <option value="other">{{ $t("option_other") }}</option>
                </select>
              ...
              <div>
                <label class="block mb-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ $t("label_full_address") }}</label>
                <textarea required rows="3" v-model="form.address_location" class="w-full p-3 text-sm font-medium border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#006A4E] outline-none resize-none bg-white transition-all"></textarea>
              </div>

              <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                  <label class="block mb-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ $t("label_region") }}</label>
                  <input type="text" required v-model="form.region" class="w-full p-3 text-sm font-medium border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#006A4E] outline-none transition-all" />
                </div>
                <div>
                  <label class="block mb-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ $t("label_city") }}</label>
                  <input type="text" required v-model="form.city" class="w-full p-3 text-sm font-medium border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#006A4E] outline-none transition-all" />
                </div>
                <div>
                  <label class="block mb-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ $t("label_province") }}</label>
                  <input type="text" required v-model="form.province" class="w-full p-3 text-sm font-medium border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#006A4E] outline-none transition-all" />
                </div>
                <div>
                  <label class="block mb-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ $t("label_postal_code") }}</label>
                  <input type="text" required v-model="form.postal_code" class="w-full p-3 text-sm font-medium border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#006A4E] outline-none transition-all" />
                </div>
              </div>
              ...
              <div class="flex items-center gap-3 p-4 mt-2 transition-colors border border-gray-200 cursor-pointer rounded-xl bg-gray-50 hover:bg-gray-100" @click="form.is_default = !form.is_default">
                <input type="checkbox" id="is_default" v-model="form.is_default" class="w-5 h-5 rounded cursor-pointer text-[#006A4E] focus:ring-[#006A4E] accent-[#006A4E]" @click.stop />
                <label for="is_default" class="text-sm font-bold text-gray-800 cursor-pointer select-none" @click.stop>
                  {{ $t("label_set_default_address") }}
                </label>
              </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-6 mt-auto border-t border-gray-100">
              <button type="button" @click="isModalOpen = false" class="px-6 py-3 text-sm font-bold text-gray-600 transition-colors bg-gray-100 rounded-xl hover:bg-gray-200">
                {{ $t("btn_cancel") }}
              </button>
              <button type="submit" class="px-6 py-3 text-sm font-bold text-white transition-all shadow-md bg-[#006A4E] rounded-xl hover:bg-emerald-900 hover:shadow-lg">
                {{ editingId ? $t("btn_update_address") : $t("btn_save_address") }}
              </button>
            </div>
          </form>
...
// --- KALKULASI TOTAL ---
const checkoutItems = computed(() => {
  // 👇 PERBAIKAN: Tangani selectedItemIds.value vs selectedItemIds agar tidak crash
...
const saveAddress = async () => {
  try {
    ...
    // 👇 PERBAIKAN: Mencegah error mapping data
...
onMounted(async () => {
  ...
  try {
    const res = await axios.get(`${BASE_URL}/exchange-rates`);
    if (res.data?.data?.rates) { exchangeRates.value = res.data.data.rates; }
  } catch (e) {}

  try {
    const resCatalog = await axios.get(`${BASE_URL}/products`);
    const dataCat = resCatalog.data?.data?.data || resCatalog.data?.data || resCatalog.data;
    if (Array.isArray(dataCat)) { setCatalogProducts(dataCat); }
  } catch (e) {}
  ...
  try {
    ...
    try {
      ...
      console.warn("Background user sync failed");
    }

    const resAddr = await axios.get(`${BASE_URL}/addresses`, getAxiosConfig());
    
    // 👇 PERBAIKAN: Mencegah DOM crash karena Object
    const addrData = resAddr.data?.data !== undefined ? resAddr.data.data : resAddr.data;
    addresses.value = Array.isArray(addrData) ? addrData : [];
    ...
    // 👇 PERBAIKAN: Pengecekan selectedItemIds dengan cara aman
    const ids = selectedItemIds?.value || selectedItemIds || [];
    if (ids.length === 0) {
      router.push(`/cart`);
    } else {
...
const applyPromo = async () => {
  ...
  try {
    ...
    const res = await axios.post(
      `${BASE_URL}/promo/verify`,
      { promo_code: codeToBeApplied, cart_items: checkoutItems.value.map((item) => ({ product_id: item.product_id })) },
      getAxiosConfig()
    );
...
onMounted(async () => {
  ...
  try {
    ...
    console.warn("Failed fetch exchange rates");
  }
  ...
  try {
    ...
    console.warn("Failed fetch catalog");
  }
  ...
  try {
    ...
    if (resUser.data) {
      ...
      setAvailablePoints(resUser.data.point || 0);
      setUserType(resUser.data.usertype || "user");
      localStorage.setItem("user_data", JSON.stringify(resUser.data));
    }
  } catch (e) {
    console.warn("Background user sync failed");
  }
  ...
  try {
    ...
    console.warn("Failed fetch addresses", e);
  }
            ...
            <form @submit.prevent="saveAddress" class="space-y-5">
              ...
              >
                ...
                <div class="relative z-0 w-full h-40 sm:h-56">
                  ...
                  <div
                    class="absolute z-[400] px-2 py-1 font-mono text-[9px] text-gray-600 bg-white/90 rounded shadow bottom-2 right-2 backdrop-blur po\inter-events-none border border-gray-200"
                  >
                    {{ form.latitude ? parseFloat(form.latitude).toFixed(5) : "-" }},
                    {{ form.longitude ? parseFloat(form.longitude).toFixed(5) : "-" }}
                  </div>
...
onMounted(async () => {
  ...
  try {
    ...
    if (resUser.data) {
      userData.value = resUser.data;
      availablePoints.value = resUser.data.point || 0;
      userType.value = resUser.data.usertype || "user";
      localStorage.setItem("user_data", JSON.stringify(resUser.data));
    }
  } catch (e) {}
...
<!-- <template>
  ...
  >
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="relative w-full max-w-5xl bg-white shadow-2xl rounded-3xl animate-fade-in flex flex-col max-h-[90vh] md:max-h-[85vh]"
        >
          <div
            class="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50 shrink-0 md:p-8 md:pb-6"
          >
            ...
            >
              ...
              <div>
                ...
                <textarea
                  ...
                  v-model="form.address_location"
                  placeholder="Street name, building, house number..."
                  class="w-full p-3 text-sm font-medium transition-all bg-white border border-gray-300 resize-none rounded-xl focus:ring-2 focus:ring-[#006A4E] outline-none custom-scrollbar"
                ></textarea>
              </div>
...
// ==========================================
// 👇 [PERBAIKAN 1]: FUNGSI VALIDASI TANGGAL DISKON 👇
// ==========================================
...
const getDiscountStatus = (p) => {
  ...
  // Jika tidak punya start/end date, asumsikan diskon selalu aktif (selama nominal diskon ada)
...
const getActivePriceObj = (product) => {
  ...
  // 👇 [PERBAIKAN 2]: Jangan ambil diskon jika getDiscountStatus bernilai false
...
const checkoutTotalIDR = computed(() => {
  return checkoutItems.value.reduce((sum, item) => {
    ...
    // 👇 [PERBAIKAN 3]: Cek durasi sebelum menerapkan diskon IDR
...
/* 👇 PERBAIKAN: Memaksa z-index peta leaflet agar tidak merusak modal 👇 */
...
// 👇 [PERBAIKAN] Gunakan field region agar tersimpan benar di DB
const form = ref({
  id: null,
  region: "Indonesia",
  first_name_address: "",
...
import { BASE_URL } from "../../config/api.js";
// import { useCart } from "../../composables/useCart.js";
import { useCart, getActivePrice, getBundlePromo } from "../../composables/useCart";
import { Country, State } from "country-state-city";
import "leaflet/dist/leaflet.css";
import { LMap, LTileLayer, LMarker } from "@vue-leaflet/vue-leaflet";
import L from "leaflet";
...
<template>
  <div v-if="isPageLoading" class="z-[100] fixed inset-0 flex flex-col justify-center items-center bg-white">
    ...
    <p class="font-serif text-sm italic tracking-widest text-gray-500 animate-pulse">{{ $t("payment.prepare_checkout") }}</p>
  </div>

  <div v-else class="relative z-10 max-w-6xl min-h-screen px-6 py-12 mx-auto md:py-24 animate-fade-in">
    <div v-if="checkoutItems.length === 0" class="py-20 text-center">
      <h2 class="mb-4 font-serif text-3xl">{{ $t("payment.bag_empty") }}</h2>
      <button @click="$router.push('/collections')" class="px-8 py-3 text-xs font-bold tracking-widest text-white uppercase bg-black rounded-full">{{ $t("payment.return_shop") }}</button>
    </div>

    <div v-else>
      <h1 class="mb-12 font-serif text-3xl tracking-tighter uppercase md:text-4xl">{{ $t("payment.checkout") }}</h1>
      ...
      <div class="flex flex-col gap-12 lg:flex-row">
        ...
        <div class="flex-grow space-y-12">
          ...
          <section>
            <div class="flex items-center gap-4 mb-4">
              <span class="flex justify-center items-center bg-black rounded-full w-6 h-6 font-bold text-[10px] text-white">1</span>
              <h2 class="text-sm font-bold tracking-widest text-gray-900 uppercase">{{ $t("payment.shipping_address") }}</h2>
            </div>

            <div v-if="addresses.length === 0" class="py-10 text-center border border-gray-300 border-dashed bg-gray-50 rounded-2xl">
              <p class="mb-2 text-sm italic text-gray-500">{{ $t("payment.no_address_found") }}</p>
              <button @click="isModalOpen = true" class="text-xs font-bold text-blue-600 underline">{{ $t("payment.add_new_address") }}</button>
            </div>
            <div v-else class="space-y-4">
              ...
              <button @click="isModalOpen = true" class="mt-4 text-xs font-bold text-gray-500 underline hover:text-black">{{ $t("payment.add_another_address") }}</button>
            </div>
          </section>
              ...
              <div v-if="shippingMethod === 'biteship'" class="p-6 mt-4 space-y-8 bg-white border border-gray-200 rounded-3xl animate-fade-in">
                ...
                <div v-if="destinationInfo?.country === 'Indonesia'">
                  ...
                  <div v-if="deliveryType === 'scheduled'" class="flex gap-4 p-4 border border-blue-100 bg-blue-50/30 rounded-xl animate-fade-in">
                    <div class="flex-1"><input type="date" v-model="deliveryDate" :min="todayDate" class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg outline-none focus:ring-black focus:border-black" required /></div>
                    <div class="flex-1"><input type="time" v-model="deliveryTime" class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg outline-none focus:ring-black focus:border-black" required /></div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div v-if="isLoadingRates" class="py-4 text-sm text-center text-gray-500 animate-pulse">{{ $t("payment.calculating_couriers") }}</div>
                  <div v-else-if="processedShippingRates.length === 0" class="py-4 text-xs italic text-center text-red-500">{{ $t("payment.no_courier_available") }}</div>
                  <div v-else class="space-y-3">
                    <label v-for="(rate, idx) in processedShippingRates" :key="idx" :class="[rate.is_disabled ? 'opacity-40 bg-gray-100 border-gray-200 pointer-events-none select-none' : selectedRate?.company === rate.company && selectedRate?.type === rate.type ? 'border-black bg-gray-50 shadow-sm' : 'border-gray-200 hover:bg-gray-50 cursor-pointer transition-all']" class="relative flex flex-col p-4 border rounded-xl">
                      <div class="flex items-center w-full">
                        ...
                        <div class="flex items-center flex-grow gap-4 ml-4">
                          <div class="flex items-center justify-center w-12 h-12 overflow-hidden bg-white border border-gray-100 rounded-lg shrink-0">
                            <span class="text-xs font-black text-gray-400">{{ rate.company.toUpperCase() }}</span>
                          </div>
                          <div>
                        ...
                        <p class="text-sm font-black text-black">{{ formatCurrencyDisplay(convertIDRtoActiveCurrency(rate.price)) }}</p>
                      </div>
                    </label>
                  </div>
                </div>
        ...
        <div class="lg:w-[400px] space-y-6">
          <div class="sticky p-8 bg-white border border-gray-100 shadow-xl rounded-3xl top-28">
            <h2 class="pb-4 mb-6 text-sm font-bold tracking-widest text-gray-900 uppercase border-b border-gray-200">{{ $t("payment.order_summary") }}</h2>
            ...
            <div class="space-y-4 mb-8 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              <div v-for="item in checkoutItems" :key="item.id" class="flex gap-4">
                ...
                <div class="flex-grow">
                  ...
                  <p class="mt-1 text-xs font-medium text-gray-900">{{ formatCurrencyDisplay({ value: getActivePriceObj(item.product).value * item.quantity, curr: getActivePriceObj(item.product).curr }) }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="pt-4 space-y-3 text-sm border-t border-gray-50">
              <div class="flex justify-between text-gray-500">
                <span>{{ $t("payment.subtotal") }}</span>
                <span>{{ formatCurrencyDisplay(cartSubtotalObj) }}</span>
              </div>

              <!-- Promo Section -->
              <div class="pt-4 mt-2 border-t border-gray-200 border-dashed">
                <label class="text-[10px] font-bold text-gray-900 uppercase tracking-widest mb-2 block">{{ $t("payment.promo_code") }}</label>
                <form @submit.prevent="applyPromo" class="flex gap-2">
                  <input type="text" v-model="promoInput" :disabled="appliedPromoCode !== null || isVerifyingPromo || useMemberVoucher" placeholder="Enter promo code" class="flex-1 bg-white border border-gray-300 rounded-lg px-3 py-1.5 text-sm uppercase focus:ring-black outline-none disabled:bg-gray-100 disabled:text-gray-400" />
                  ...
                  <button v-else type="button" @click="removePromo" class="bg-red-50 text-red-600 border border-red-200 text-[10px] font-bold uppercase px-4 rounded-lg w-20">{{ $t("payment.remove") }}</button>
                </form>
                <div v-if="appliedPromoCode" class="flex justify-between text-[10px] font-medium text-emerald-600 mt-2">
              ...
              <div v-if="userData?.is_membership && availablePoints > 0" class="pt-4 mt-2 border-t border-gray-200 border-dashed">
                <div class="flex justify-between mb-2">
                  <span class="text-[10px] font-bold text-yellow-800 uppercase tracking-widest">Redeem Points</span>
                  <span class="text-xs text-gray-500">{{ availablePoints }} Pts</span>
                </div>
                <div class="flex gap-2">
                  <input type="number" v-model.number="pointsToUse" :max="maxUsablePoints" min="0" :disabled="['SOLHOST34', 'MERDEKA17'].includes(appliedPromoCode)" class="flex-1 bg-white border border-yellow-300 rounded-lg px-3 py-1.5 text-sm outline-none disabled:bg-gray-100" />
                  <button type="button" @click="pointsToUse = maxPointsAllowed" :disabled="['SOLHOST34', 'MERDEKA17'].includes(appliedPromoCode)" class="bg-yellow-100 text-yellow-800 text-[10px] font-bold uppercase px-3 rounded-lg hover:bg-yellow-200">Use All</button>
                </div>
                <p v-if="pointsToUse > 0" class="text-[10px] text-green-600 mt-1 font-medium text-right">- {{ formatCurrencyDisplay(appliedPointDiscountObj) }}</p>
              </div>

              <div class="flex justify-between text-gray-500">
                <span>{{ $t("payment.shipping") }}</span>
                <span v-if="shippingMethod === 'free'" class="font-bold text-green-600">Free</span>
                <span v-else-if="shippingMethod === 'biteship' && selectedRate" class="font-medium text-gray-900">{{ formatCurrencyDisplay(shippingCostObj) }}</span>
              </div>

              <div class="flex justify-between pt-4 font-bold text-gray-900 border-t border-gray-200">
                <span class="mt-1 text-xs tracking-widest uppercase">{{ $t("payment.grand_total") }}</span>
                <span class="text-xl">{{ formatCurrencyDisplay(grandTotalObj) }}</span>
              </div>
...
const {
  cartItems, bundleDiscountAmount, checkoutCount, checkoutTotalAmount, selectedItemIds, clearSelectedCart,
} = useCart();
...
const getPriceToDisplay = (product) => {
  ...
  try {
    const pricesObj = typeof product.prices === "string" ? JSON.parse(product.prices) : product.prices || {};
    const dbPrice = pricesObj[curr] || pricesObj[curr.toLowerCase()];
    if (dbPrice) return { value: parseFloat(dbPrice), curr: curr };
  } catch (e) {}
  return { value: Number(product.price), curr: "IDR" };
};
...
const getActivePriceObj = (product) => {
  ...
  return dynamicPriceObj; // Diskon asumsikan ditangani di keranjang utama
};
...
const checkoutTotalIDR = computed(() => {
  return checkoutItems.value.reduce((sum, item) => {
    return sum + (getActivePriceObj(item.product).value * item.quantity);
  }, 0);
});
...
const grandTotalObj = computed(() => {
  const calc = cartSubtotalObj.value.value - bundleDiscountAmount.value + shippingCostObj.value.value - actualPromoDiscountObj.value.value - appliedPointDiscountObj.value.value;
  return { value: Math.max(0, calc), curr: currentCurrency.value };
});

const parseColorName = (colorString) => colorString ? colorString.split("|")[0] : "";
...
const onAddressSaved = async (newId) => {
  ...
  selectedAddressId.value = newId || (addresses.value[0]?.id);
  isModalOpen.value = false;
};
...
watch(selectedAddressId, async (newVal) => {
  if (newVal) {
    ...
    try {
      const res = await axios.post(`${BASE_URL}/shipping/rates`, { address_id: newVal, cart_ids: ids }, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
      rawShippingRates.value = res.data.data || res.data.rates || res.data.pricing || [];
    } catch (error) {} finally { isLoadingRates.value = false; }
  }
});

const processedShippingRates = computed(() => {
  if (!rawShippingRates.value) return [];
  return rawShippingRates.value.map((rate) => ({
    ...rate,
...
const applyPromo = async () => {
  ...
  try {
    ...
    promoSuccess.value = false;
    promoMessage.value = "❌ " + (error.response?.data?.message || "Invalid promo.");
    appliedPromoCode.value = null;
  } finally {
    isVerifyingPromo.value = false;
  }
...
const removePromo = () => { promoInput.value = ""; appliedPromoCode.value = null; promoDiscountAmount.value = 0; promoMessage.value = ""; };
const useAllPoints = () => { pointsToUse.value = maxPointsAllowed.value; };
...
const handlePayment = async () => {
  ...
  try {
    ...
    // SECURITY NOTE: Nilai use_points dan promo_code dikirim dari frontend. 
    // Backend Controller WAJIB memvalidasi ulang kebenaran saldo poin pengguna
    // dan validitas promo code agar tidak terjadi manipulasi angka via DevTools.
    ...
    if (error.response?.status === 429) {
      Swal.fire({ icon: 'warning', title: 'Antrean Padat!', text: error.response.data.message, confirmButtonColor: '#000' });
    } else {
...
onMounted(async () => {
  window.addEventListener("currency-changed", updateCurrencyState);
  try {
    const tokenConfig = { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } };
    const [resExchange, resCatalog, resUser] = await Promise.allSettled([
      axios.get(`${BASE_URL}/exchange-rates`), axios.get(`${BASE_URL}/products`), axios.get(`${BASE_URL}/user`, tokenConfig),
    ]);
    ...
    if (resUser.status === "fulfilled" && resUser.value.data) {
      userData.value = resUser.value.data; availablePoints.value = userData.value.point || 0; userType.value = userData.value.usertype || "user";
    }
    await fetchAddresses();
    if (addresses.value.length > 0) selectedAddressId.value = addresses.value.find((a) => a.is_default)?.id || addresses.value[0].id;
  } catch (error) {} finally { isPageLoading.value = false; }
});
onUnmounted(() => window.removeEventListener("currency-changed", updateCurrencyState));
</script>

<style scoped>
/* Transisi dan Scrollbar bawaan */
