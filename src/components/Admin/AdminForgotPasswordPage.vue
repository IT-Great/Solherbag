  ...
  <div class="flex justify-center items-center bg-[#1F2937] px-6 min-h-screen">
    <div class="bg-white shadow-sm p-10 border border-gray-300 rounded-[3rem] w-full max-w-md">
      ...
      <p class="text-sm text-gray-500 text-center mb-8">
        Enter your admin email address, and we'll send you a verification code.
      </p>

      <form @submit.prevent="handleSendCode" class="space-y-6">
        <div class="flex flex-col">
          <label class="mb-2 font-bold text-black text-sm">Admin Email Address</label>
          <input
            type="email"
            v-model="email"
        ...
        >
          <span v-if="!isLoading">Send Recovery Code</span>
          <div v-else class="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
        </button>
      </form>
      
      <div class="text-center mt-6">
        <router-link to="/loginadmin" class="text-xs text-gray-500 hover:text-black hover:underline transition">Back to Admin Login</router-link>
      </div>
    </div>
  </div>
</template>
...
const handleSendCode = async () => {
  ...
  try {
    await axios.post(`${BASE_URL}/admin/forgot-password/send-code`, { email: email.value });
