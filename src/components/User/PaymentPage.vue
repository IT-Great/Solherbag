...
// [BARU] Fungsi pembantu untuk mengubah format item keranjang ke format GTM
...
const fetchData = async () => {
  try {
    ...
    await nextTick();
    // [BARU] 3. Event: Add Payment Info / Begin Checkout (GTM)
    // Dipanggil saat halaman checkout selesai di-render
...
const handlePayment = async () => {
  ...
  try {
    ...
    if (res.data.checkout_url) {
      // [BARU] 4. Event: Purchase (GTM)
      // Dipanggil sesaat sebelum redirect ke halaman payment gateway
      trackGtmEvent("purchase", {
        ecommerce: {
          transaction_id: res.data.order_id || res.data.invoice_id || `ORD-${Date.now()}`, // Fallback jika backend tidak membalikkan ID
              ...
              <div v-if="userData?.is_membership" class="pt-4 mt-2 border-t border-gray-200 border-dashed">
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <p class="text-[10px] text-gray-500 mt-0.5">Use 500k Member Voucher</p>
                    <p v-if="checkoutTotalAmount < MEMBER_MIN_SPEND" class="text-[8px] text-red-500 italic mt-0.5">Min. spend Rp 1.000.000</p>
                  </div>
                  ...
                  <label class="relative inline-flex items-center cursor-pointer">
                    <input 
                      ...
                      class="sr-only peer" 
                      :disabled="isVerifyingPromo || (checkoutTotalAmount < MEMBER_MIN_SPEND && !useMemberVoucher)"
                    >
  ...
  if (useMemberVoucher.value) {
    ...
    if (checkoutTotalAmount.value < MEMBER_MIN_SPEND) {
      Swal.fire({
        ...
        position: 'top-end',
        icon: 'warning',
        title: `Min. spend to use member voucher is Rp ${MEMBER_MIN_SPEND.toLocaleString('id-ID')}`,
        showConfirmButton: false,
        timer: 3000
      });
      // Batalkan toggle
