...
<template>
  <div>
    <div v-show="isMobileOpen" @click="closeMobileSidebar" class="fixed inset-0 z-40 transition-opacity bg-black/50 md:hidden backdrop-blur-sm"></div>

    <aside :class="[ isCollapsed ? 'md:w-20' : 'md:w-64', isMobileOpen ? 'translate-x-0' : '-translate-x-full', 'fixed inset-y-0 left-0 z-50 flex flex-col h-screen w-64 md:sticky md:top-0 md:translate-x-0 overflow-hidden transition-all duration-300 ease-in-out bg-white border-r border-gray-200 shadow-xl md:shadow-none']">
      <div class="flex items-center justify-between p-6 border-b border-gray-100 shrink-0">
        <div class="flex justify-center flex-grow">
          <img src="../../../assets/solherbrandbook.png" alt="Logo" :class="[isCollapsed ? 'h-8' : 'h-16']" class="w-auto transition-all duration-300" />
        </div>
        <button @click="closeMobileSidebar" class="p-2 text-gray-500 md:hidden hover:text-black">
      ...
      <div v-show="!isCollapsed || isMobileOpen" class="mt-4 text-center transition-opacity duration-300 shrink-0">
        <p class="text-xs tracking-widest text-gray-400 uppercase">Administrator</p>
        <h4 class="text-sm font-bold text-black">Hi, {{ userName }}</h4>
      </div>
      ...
      <nav class="flex-grow pb-4 mt-6 overflow-y-auto custom-scrollbar">
        <ul class="px-3 space-y-1">
          <template v-for="(item, index) in filteredMenuItems" :key="index">
            <li v-if="item.type === 'label'" class="pt-4 pb-1 pl-3">
              <span v-show="!isCollapsed || isMobileOpen" class="text-[10px] font-black text-gray-400 uppercase tracking-widest transition-opacity duration-300">{{ item.name }}</span>
              <div v-show="isCollapsed && !isMobileOpen" class="h-px mx-2 mt-4 bg-gray-200"></div>
            </li>

            <li v-else-if="item.children">
              <button @click="toggleDropdown(item.name)" :class="['group flex items-center justify-between w-full p-3 rounded-xl text-gray-700 transition-colors hover:bg-gray-100', isDropdownOpen(item.name) ? 'bg-gray-50' : '']">
                <div class="flex items-center">
                  <div class="flex-shrink-0" v-html="item.icon"></div>
                  <span v-show="!isCollapsed || isMobileOpen" class="ml-4 text-sm font-medium transition-opacity duration-300 whitespace-nowrap">{{ item.name }}</span>
                </div>
                <svg v-show="!isCollapsed || isMobileOpen" :class="isDropdownOpen(item.name) ? 'rotate-180' : ''" class="w-4 h-4 text-gray-400 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
              </button>

              <ul v-show="(!isCollapsed || isMobileOpen) && isDropdownOpen(item.name)" class="relative pr-2 mt-1 mb-2 space-y-1 pl-11">
                <div class="absolute top-0 w-px bg-gray-200 left-6 bottom-2"></div>
                <li v-for="(child, childIndex) in item.children" :key="childIndex" class="relative">
                  <div class="absolute left-[-20px] top-1/2 w-3 h-px bg-gray-200"></div>
                  <router-link :to="child.path" @click="closeMobileSidebar" class="flex items-center px-3 py-2 text-xs text-gray-500 transition-colors rounded-lg hover:text-black hover:bg-gray-100" active-class="font-bold text-blue-600 bg-blue-50">{{ child.name }}</router-link>
                </li>
              </ul>
            </li>

            <li v-else>
              <router-link :to="item.path" @click="closeMobileSidebar" class="flex items-center p-3 text-gray-700 transition-colors group hover:bg-gray-100 rounded-xl" active-class="font-semibold text-blue-600 bg-blue-50">
                <div class="flex-shrink-0" v-html="item.icon"></div>
                <span v-show="!isCollapsed || isMobileOpen" class="ml-4 text-sm transition-opacity duration-300 whitespace-nowrap">{{ item.name }}</span>
              </router-link>
            </li>
          </template>
        </ul>
      ...
      <div class="p-4 border-t border-gray-100 shrink-0">
        <button @click="handleLogout" class="flex items-center w-full p-3 text-red-500 transition-colors group hover:bg-red-50 rounded-xl">
...
const menuItems = [
  { type: "label", name: "Menu" },
  { name: "Dashboard", path: "/admin/dashboard", moduleId: "dashboard", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2H6a2 2 0 01-2-2v-4zM14 16a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2v-4z"></path></svg>' },
  { name: "Categories", path: "/admin/categories", moduleId: "categories", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"></path></svg>' },
  {
    name: "Products",
    icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>',
    children: [
  ...
  { name: "Transactions", path: "/admin/transactions", moduleId: "transactions", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>' },
  { name: "Messages", path: "/admin/messages", moduleId: "messages", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>' },
  { name: "Sales Reports", path: "/admin/salesreports", moduleId: "sales_report", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2m0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>' },
  { name: "Users", path: "/admin/user_list", moduleId: "users", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>' },
  { name: "Subscribers", path: "/admin/subscribers", moduleId: "subscribers", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>' },
  { name: "Events", path: "/admin/events", moduleId: "events", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>' },
  { name: "Audit Trail", path: "/admin/audit-logs", moduleId: "audit_logs", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg>' },
  { name: "Afilliate Management", path: "/admin/affiliates", moduleId: "affiliates", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>' },
  { type: "label", name: "Accounting" },
  { name: "COA", path: "/admin/coas", moduleId: "accounting_mod", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"></path></svg>' },
  { name: "Payments", path: "/admin/payments", moduleId: "accounting_mod", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path></svg>' },
  { name: "Suppliers", path: "/admin/suppliers", moduleId: "accounting_mod", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>' },
  { name: "Invoices", path: "/admin/invoices", moduleId: "accounting_mod", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>' },
  { type: "label", name: "System Settings" },
  { name: "Access Policy", path: "/admin/access-policy", moduleId: "system_policy", icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg>' }
];
...
const handleLogout = () => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire({ title: "Logged Out!", text: "See you again.", icon: "success", timer: 1500, showConfirmButton: false }).then(() => {
        router.push("/loginadmin");
      });
    }
  });
