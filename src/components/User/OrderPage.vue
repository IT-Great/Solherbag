            ...
            <div v-else-if="order.shipping_method === 'biteship' && order.courier_company" class="flex items-center gap-3">
              ...
              <div>
                ...
                <p v-else class="text-[10px] text-gray-500 mt-0.5">
                  Resi:
                  <span class="font-mono font-bold text-black">{{
                    order.biteshipData?.courier?.waybill_id || order.tracking_number || "Waiting..."
                  }}</span>
                </p>
              </div>
            </div>
...
const filteredTransactions = computed(() => {
  return transactions.value.filter(order => {
    ...
    // 2. Shipping Status Match (Using Biteship Data)
    let matchShipping = false;
    // Jika filter shipping "all" ATAU order bukan biteship (internal free), abaikan filter shipping
    if (activeShippingTab.value === 'all' || order.shipping_method !== 'biteship') {
      matchShipping = true;
    } else {
      // Dapatkan status shipping dari data Biteship yang di-fetch secara dinamis
      ...
      if (activeShippingTab.value === 'placed') {
         ...
         // In transit mencakup picked dan dropping off
...
const fetchBiteshipTrackingForOrder = async (order) => {
  ...
  try {
    ...
    const res = await axios.get(`${BASE_URL}/transactions/${order.id}/tracking`, config);
        ...
        >
          ...
          <div class="flex flex-col items-end w-full gap-2 md:w-auto">
            ...
            >
              ...
              >
              <span
                v-if="order.biteshipDataLoading"
                class="text-[10px] text-gray-400 italic animate-pulse"
                >Loading...</span
              >
              <span
                v-else
                :class="shippingStatusClass(order.biteshipData?.status)"
...
const canRequestRefund = (order) => {
  ...
  // Jika pakai Biteship, kita cek status kurirnya
  if (order.shipping_method === "biteship") {
    const shipStatus = order.biteshipData?.status
      ? order.biteshipData.status.toLowerCase()
      : "pending";

    // Jika kurir SUDAH MENGAMBIL BARANG (picked, dropping_off, delivered),
    // user TIDAK BOLEH lagi menekan tombol refund sembarangan.
    // Mereka harus menghubungi CS untuk melakukan proses "Return".
    ...
    if (unRefundableLogistics.includes(shipStatus)) {
      return false; // Sembunyikan tombol
    }

    // Jika status kurir masih 'allocated' atau 'picking_up' (kurir belum pegang barang), boleh refund
    return true;
  }
...
const transactionTabs = [
  { label: "All", value: "all" },
  { label: "Pending", value: "pending" }, // Kita sisakan 'pending' untuk payment yang belum dibayar tapi invoice sudah tergenerate di Xendit (ini bukan awaiting_payment dari keranjang)
  { label: "Processing", value: "processing" },
  { label: "Completed", value: "completed" },
  { label: "Cancelled", value: "cancelled" },
  { label: "Refund Issues", value: "refund" },
...
// [PERBAIKAN] Tidak ada lagi awaiting payment. Yang bisa dibayar hanya 'pending' (yaitu Invoice yang aktif)
...
// [PERBAIKAN] Hilangkan style untuk 'awaiting_payment'
const statusClass = (status) => {
  const map = {
    pending: "bg-orange-100 text-orange-700",
    processing: "bg-blue-100 text-blue-700",
...
// [PERBAIKAN] Fungsi Penarik Data Kurs Dinamis
...
// [PERBAIKAN] Fungsi Format Harga Cerdas
const formatLocalPrice = (value, order) => {
