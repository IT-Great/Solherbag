            ...
            <td class="py-4 pr-2 text-center">
              <div class="flex items-center justify-center gap-2">
                ...
                <button
                  ...
                  @click.stop="$router.push(`/admin/chat/${user.id}`)"
                  class="p-2 text-gray-600 transition-colors bg-gray-100 rounded-lg hover:bg-green-50 hover:text-green-600"
                  title="Chat with User"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
...
// Kita bedakan permission untuk melihat detail user vs membuka chat
const myPermissions = ref({
  viewDetails: false, // Diambil dari modul 'users' (Read)
  chat: false, // Diambil dari modul 'messages' (Read / Menu)
});
...
// [BARU] Mengambil data hak akses pengguna yang sedang login (Menyilangkan 2 Modul)
const fetchUserPermissions = async () => {
  ...
  try {
    // Kita gunakan cache lokal jika ada, atau tembak API jika kosong
    // (Sesuai dengan praktik di AdminHeader.vue sebelumnya agar cepat)
    let policies = null;
    ...
    // Tarik rule dari modul "User Management"
    const userTerms = myRolePolicies.users || [];

    // Tarik rule dari modul "Messages & Inbox"
    const messageTerms = myRolePolicies.messages || [];

    myPermissions.value = {
      // Boleh lihat detail jika punya akses R (Read) di modul users
      viewDetails: userTerms.includes("read"),

      // Boleh chat jika punya akses M (Menu) atau R (Read) di modul messages
...
onMounted(() => {
  // Rantai proses: Cek akses dulu, baru ambil data tabel
...
// 👇 PERBAIKAN: Logika Cerdas untuk Mengapungkan Pelanggan yang Chat-nya Belum Dibaca ke Posisi Teratas
const sortedAndFilteredUsers = computed(() => {
  ...
  // Mengurutkan berdasarkan unread_count secara descending (Paling banyak unread = Nomor 1)
  return result.slice().sort((a, b) => {
    const aUnread = a.unread_count || 0;
    const bUnread = b.unread_count || 0;

    // Jika jumlah unread beda, prioritaskan yang ada angkanya
    if (aUnread !== bUnread) {
      return bUnread - aUnread;
    }
    // Jika sama, urutkan berdasarkan data yang paling baru daftar
