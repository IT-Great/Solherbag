<template>
  <div class="mx-auto px-6 py-12 max-w-7xl min-h-screen animate-fade-in">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
      <div>
        <h1 class="font-bold text-gray-800 text-2xl">Shipping & Logistics (Simulator)</h1>
        <p class="text-gray-500 text-sm">Monitor and manually simulate courier statuses for testing purposes.</p>
      </div>
    ...
    <div v-if="isLoading" class="py-20 flex flex-col items-center justify-center animate-pulse">
      <div class="border-4 border-gray-100 border-t-black rounded-full w-10 h-10 animate-spin mb-4"></div>
      <p class="font-bold text-gray-400 text-xs uppercase tracking-widest">Loading Logistics Data...</p>
    </div>

    <div v-else class="bg-white shadow-sm border border-gray-100 rounded-2xl overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-100 text-gray-500 text-[10px] uppercase tracking-widest">
              ...
              <th class="p-4 whitespace-nowrap bg-blue-50/50">Simulation Panel</th>
            </tr>
          </thead>
          <tbody class="text-gray-700 text-sm divide-y divide-gray-50">
            <tr v-for="trx in filteredTransactions" :key="trx.id" class="hover:bg-gray-50/50 align-top transition-colors">
              
              <td class="p-4">
                <p class="font-mono font-bold text-black">{{ trx.order_id }}</p>
                <p class="text-[10px] text-gray-400 mt-1">{{ formatDate(trx.created_at) }}</p>
                <div class="mt-3">
                  <p class="font-bold text-xs">{{ trx.user.first_name }} {{ trx.user.last_name }}</p>
                  <p class="text-[10px] text-gray-500 truncate w-32" :title="trx.user.email">{{ trx.user.email }}</p>
                </div>
              </td>

              <td class="p-4">
                <div v-if="trx.address">
                  <p class="font-bold text-xs">{{ trx.address.first_name_address }} {{ trx.address.last_name_address }}</p>
                  ...
                  <p class="text-[10px] text-gray-500 mt-2 bg-gray-100 px-2 py-1 rounded inline-block font-mono">
                    POSTAL: {{ trx.address.postal_code }}
                  </p>
                </div>
                <div v-else class="text-xs text-gray-400 italic">Address data missing</div>
              </td>
              ...
              <td class="p-4">
                <div class="space-y-2 text-xs">
                  <div class="flex justify-between items-center border-b border-gray-100 pb-1">
                    ...
                    <span class="font-bold">{{ calculateWeight(trx) }} gram</span>
                  </div>
                  <div class="flex justify-between items-center border-b border-gray-100 pb-1">
                    <span class="text-gray-500">Shipping Cost:</span>
                    <span class="font-bold text-green-600">{{ formatPrice(trx.shipping_cost) }}</span>
                  </div>
                  <div class="flex justify-between items-center pt-1">
                    <span class="text-gray-500 text-[10px] uppercase">Main Status:</span>
                    <span :class="statusClass(trx.status)" class="text-[9px] font-bold uppercase px-2 py-0.5 rounded-full">{{ trx.status.replace('_', ' ') }}</span>
                  </div>
                </div>
              </td>

              <td class="p-4">
                <div v-if="trx.shipping_method === 'biteship'" class="space-y-2">
                  <p class="font-bold text-xs uppercase">{{ trx.courier_company }} - <span class="font-normal">{{ trx.courier_type }}</span></p>
                  <div class="bg-gray-100 p-2 rounded-lg text-[10px] font-mono break-all">
                    <span class="text-gray-400 block mb-0.5">Biteship ID:</span>
                    {{ trx.biteship_order_id || 'Not Generated' }}
                  </div>
                </div>
                <div v-else class="text-xs font-bold text-gray-500 uppercase flex items-center gap-2">
              ...
              <td class="p-4 bg-blue-50/20 border-l border-blue-50">
                <div v-if="trx.shipping_method === 'biteship'" class="space-y-3">
                  
                  <div>
                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Tracking Number (Resi)</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Shipping Status</label>
                    <select 
                    ...
                    >
                      ...
                      <option value="allocated">Allocated (Kurir Ditemukan)</option>
                      <option value="picking_up">Picking Up (Kurir Menuju Toko)</option>
                      <option value="picked">Picked (Barang Diambil)</option>
                      <option value="dropping_off">Dropping Off (Kurir Menuju Tujuan)</option>
                ...
                <div v-else class="text-[10px] text-gray-400 italic text-center py-4">
            ...
            <tr v-if="!isLoading && filteredTransactions.length === 0">
              <td colspan="5" class="py-20 font-serif text-gray-400 text-center italic">
...
const fetchLogistics = async () => {
  ...
  try {
    ...
    const validTransactions = res.data.filter(t => t.status !== 'awaiting_payment' && t.status !== 'pending');
...
const calculateWeight = (trx) => {
  ...
  const totalQty = trx.details.reduce((sum, item) => sum + parseInt(item.quantity), 0);
