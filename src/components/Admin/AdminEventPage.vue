            ...
            >
              ...
              <td class="px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  ...
                  <span
                    v-if="!canRead && !canUpdate && !canDelete && !isSuperAdmin"
...
const canRead = computed(() => myPermissions.value.read);
...
// [BARU] Fungsi mengambil data hak akses dari cache / API
const fetchUserPermissions = async () => {
  ...
  try {
    ...
    const eventTerms = myRolePolicies.events || [];

    myPermissions.value = {
      create: eventTerms.includes("create"),
      read: eventTerms.includes("read"),
      update: eventTerms.includes("update"),
      delete: eventTerms.includes("delete"),
    };
  } catch (error) {
