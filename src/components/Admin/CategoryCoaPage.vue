<template>
  <div class="relative bg-white shadow-sm p-8 border border-gray-100 rounded-2xl min-h-[500px]">
    <div class="flex justify-between items-center mb-8">
      ...
      <button @click="openModal()" class="bg-black hover:bg-gray-800 px-6 py-2 rounded-xl font-semibold text-white transition">
        + Add Category
      </button>
    </div>
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="text-gray-700 text-sm">
          <tr v-for="cat in categories" :key="cat.id" class="border-b border-gray-50 hover:bg-gray-50 transition">
            <td class="py-4 font-mono">{{ cat.id }}</td>
            <td class="py-4 font-bold">{{ cat.category_name }}</td>
            <td class="py-4 text-right">
              <button @click="openModal(cat)" class="text-amber-500 hover:underline font-medium mr-4">Edit</button>
              <button @click="confirmDelete(cat.id)" class="text-red-500 hover:underline font-medium">Delete</button>
            </td>
          </tr>
          <tr v-if="categories.length === 0">
            <td colspan="3" class="text-center py-10 text-gray-400 italic">No COA categories found.</td>
          </tr>
        </tbody>
      </table>
    </div>

    <div v-if="showModal" class="z-50 fixed inset-0 flex justify-center items-center bg-black/50 backdrop-blur-sm p-4">
      <div class="bg-white shadow-2xl p-8 rounded-3xl w-full max-w-md">
        <h2 class="mb-6 font-bold text-xl">{{ isEdit ? "Edit Category" : "New Category" }}</h2>
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div>
            <label class="block mb-1 font-bold text-sm">Category Name</label>
            <input v-model="form.category_name" type="text" class="bg-gray-50 p-3 rounded-xl outline-none focus:ring-2 focus:ring-black w-full" required />
          </div>
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await axios.put(`${BASE_URL}/admin/category-coas/${form.value.id}`, form.value, axiosConfig);
    } else {
