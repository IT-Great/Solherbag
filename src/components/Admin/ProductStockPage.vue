...
<template>
  ...
  >
    ...
    >
      <div>
        <h1 class="text-2xl font-bold text-gray-800">Active Stock Management</h1>
        <p class="text-sm text-gray-500">
          Monitor and add stock batches to your <span class="font-bold text-green-600">active</span> products.
        </p>
      </div>
      <!-- 👇 TOMBOL MENUJU HALAMAN INAKTIF 👇 -->
      <router-link
        to="/admin/stocks/inactive"
...
const exportToExcel = () => {
  ...
  const worksheet = XLSX.utils.json_to_sheet(data);
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, "Active_Stock");
  XLSX.writeFile(
    workbook,
    `Solher_ActiveStock_${new Date().toISOString().split("T")[0]}.xlsx`
  );
};
...
const exportToPDF = () => {
  ...
  doc.setFontSize(16);
  doc.text("Solher - Active Stock Report", 14, 15);
  doc.setFontSize(10);
  doc.text(`Generated on: ${new Date().toLocaleString()}`, 14, 22);
