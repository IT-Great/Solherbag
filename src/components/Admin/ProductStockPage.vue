<template>
  ...
  >
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody v-if="isLoading">
          <tr v-for="i in 5" :key="`skel-${i}`" class="border-b border-gray-50">
            ...
            <td class="py-6 text-center">
              <div
                class="h-6 w-10 mx-auto bg-gray-200 rounded animate-pulse"
              ></div>
            </td>
            <td class="py-6">
              <div class="h-10 w-48 bg-gray-100 rounded-lg animate-pulse"></div>
            </td>
            <td class="py-6">
              <div
            ...
            <td class="py-6 text-center pr-2">
              <div
                class="h-8 w-24 mx-auto bg-gray-200 rounded-lg animate-pulse"
              ></div>
            </td>
          </tr>
        </tbody>
        ...
        <tbody v-else class="text-gray-600">
          <tr
            ...
            :key="product.id"
            class="border-b border-gray-50 hover:bg-gray-50 transition align-top"
          >
            ...
            <td class="py-4 w-[35%]">
              ...
              >
                <!-- <div v-for="(batch, idx) in product.stocks" :key="batch.id" class="flex justify-between items-center bg-white border border-gray-200 rounded-lg p-2 shadow-sm text-xs"> -->
                <div
                  v-for="batch in product.stocks"
                  :key="batch.id"
                  class="flex justify-between items-center bg-white border border-gray-200 rounded-lg p-2 shadow-sm text-xs"
                >
                  <div>
                    <span class="font-mono font-bold text-blue-600 block">{{
                      batch.batch_code
                    }}</span>
                  ...
                  <div
                    class="bg-gray-100 px-3 py-1 rounded font-bold text-gray-800"
                  >
                    {{ batch.quantity }} pcs
                  </div>
                </div>
              ...
              <div
                v-else
                class="text-xs text-gray-400 italic bg-gray-50 p-2 rounded-lg border border-dashed text-center"
              >
                No active batches.
              </div>
            </td>
            ...
            <td class="py-4 text-center pr-2 w-[10%]">
              <button
                ...
                class="bg-black hover:bg-gray-800 text-white px-4 py-2 rounded-xl text-xs font-bold transition shadow-sm w-full"
              >
                + Add Batch
              </button>
            </td>
    ...
    >
      <div
        class="bg-white rounded-[2rem] shadow-2xl p-8 w-full max-w-sm relative"
      >
        <button
        ...
        <form @submit.prevent="submitAddStock" class="space-y-4">
          <div>
            ...
            <input
              ...
              type="number"
              min="1"
              class="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 outline-none focus:ring-2 focus:ring-black text-xl font-black text-center"
              required
            />
          </div>
          <button
            ...
            class="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white font-bold py-3 rounded-xl transition shadow-md shadow-blue-500/30"
          >
