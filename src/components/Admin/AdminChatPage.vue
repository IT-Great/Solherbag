...
<template>
  ...
  >
    <div class="flex flex-col w-full max-w-3xl bg-[#F9FAFB] shadow-2xl rounded-2xl overflow-hidden h-[85vh] md:h-[80vh]">
      
      <div class="relative z-10 flex items-center px-6 py-4 bg-white border-b border-gray-200 shadow-sm shrink-0">
        <button @click="$router.back()" class="p-2 mr-4 text-gray-400 transition bg-gray-50 rounded-full hover:text-black hover:bg-gray-200 focus:outline-none">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        ...
        >
          ...
          >
            <div v-if="msg.attachment" class="mb-2">
              <img v-if="msg.attachment_type === 'image'" :src="getAttachmentUrl(msg.attachment)" class="max-w-full rounded-lg max-h-48 object-cover cursor-pointer" @click="openImage(getAttachmentUrl(msg.attachment))" />
