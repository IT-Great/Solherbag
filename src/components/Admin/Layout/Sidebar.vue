          ...
          <template v-for="(item, index) in filteredMenuItems" :key="index">
            ...
            <li v-else>
              <router-link
                :to="item.path"
                @click="closeMobileSidebar"
                class="flex items-center justify-between p-3 text-gray-700 transition-colors group hover:bg-gray-100 rounded-xl"
                active-class="font-semibold text-blue-600 bg-blue-50"
              >
                ...
                <transition name="pop">
                  <span
                    ...
                    "
                    class="px-2 py-0.5 ml-2 text-[10px] font-bold text-white bg-red-600 rounded-full shadow-sm whitespace-nowrap"
                  >
                    {{ totalUnreadChats }} Pesan
                  </span>
                </transition>
              </router-link>
            </li>
...
// State Baru untuk Global Radar Chat
...
// 👇 PERBAIKAN: Fungsi Menghitung Total Chat Belum Terbaca dari Semua Pelanggan
const fetchTotalUnreadChats = async () => {
  ...
  try {
    ...
    // Jumlahkan semua unread_count dari seluruh pengguna
    totalUnreadChats.value = res.data.reduce(
      (sum, user) => sum + (user.unread_count || 0),
      0
    );
  } catch (error) {
    console.error("Gagal menarik unread chat", error);
  }
};

// 👇 PERBAIKAN: Menyalakan Radar Websocket Global untuk Admin
const setupAdminGlobalPusher = async () => {
  if (!adminToken) return;
  try {
    ...
    if (resSupport.data && resSupport.data.length > 0) {
      ...
      window.Echo = new Echo({
        ...
        auth: { headers: { Authorization: `Bearer ${adminToken}` } },
      });

      // Sadap kotak pos Solher Care secara global
      window.Echo.private(`chat.${supportId.value}`).listen(".message.sent", (e) => {
        const incomingMsg = e.message || e;

        // Jika yang mengirim adalah pelanggan (bukan sesama Admin/AI)
        const myAdminId = JSON.parse(localStorage.getItem("admin"))?.id;
        if (incomingMsg.sender_id !== myAdminId) {
          fetchTotalUnreadChats(); // Update angka badge di sidebar
          window.dispatchEvent(new Event("refresh-user-list-chat")); // Suruh halaman UserList untuk bergeser
        }
      });
    }
  } catch (error) {}
...
const filteredMenuItems = computed(() => {
  ...
  if (!isSuperAdmin.value) {
    ...
    authorizedItems = clonedMenuItems.filter((item) => {
      ...
      if (item.children) {
        item.children = item.children.filter((child) =>
          rolePerms[child.moduleId]?.includes("menu")
        );
        return item.children.length > 0;
      }
      return rolePerms[item.moduleId]?.includes("menu");
...
onMounted(() => {
  ...
  fetchTotalUnreadChats(); // Tarik angka badge
  setupAdminGlobalPusher(); // Nyalakan Radar

  checkOrientation();
  checkActiveRoute();

  // Update badge jika ada perintah dari halaman lain (contoh: pesan selesai di read)
  window.addEventListener("refresh-chat-badge", fetchTotalUnreadChats);
  window.addEventListener("resize", checkOrientation);
  window.addEventListener("toggle-admin-sidebar", handleExternalToggle);
});

onUnmounted(() => {
  window.removeEventListener("refresh-chat-badge", fetchTotalUnreadChats);
  window.removeEventListener("resize", checkOrientation);
  window.removeEventListener("toggle-admin-sidebar", handleExternalToggle);
...
const menuItems = [
  ...
  // 👇 [BARU] Menu Email Campaigns 👇
  {
    name: "Email Campaigns",
    path: "/admin/newsletters",
    moduleId: "subscribers", // Meminjam permission modul subscribers agar tidak perlu atur ulang database hak akses
    icon:
    ...
    if (resSupport.data && resSupport.data.length > 0) {
      ...
      window.Echo.private(`chat.${supportId.value}`).listen(".message.sent", (e) => {
        ...
        if (incomingMsg.sender_id !== myAdminId) {
          ...
          window.dispatchEvent(new Event("refresh-user-list-chat"));
        }
      });
    }
  } catch (error) {}
...
const menuItems = [
  ...
  // 👇 MENU TELESCOPE BARU 👇
  {
    name: "Telescope Monitor",
    path: "/admin/telescope",
    moduleId: "system_policy", // Menumpang role access policy agar hanya admin yang berhak yang bisa lihat
    icon:
