<!-- <template>
  <div class="max-w-4xl px-6 py-20 mx-auto">
    <div class="p-8 bg-white border border-gray-200 shadow-sm rounded-2xl">
      <h1 class="mb-8 text-3xl font-bold">Your Profile</h1>

      <div v-if="userData" class="flex flex-col gap-10 md:flex-row">
        <div class="flex flex-col items-center gap-4">
          <div class="relative group">
            <img
              :key="userData.profile_image"
              :src="userData.profile_image"
              class="object-cover w-32 h-32 border-4 rounded-full shadow-sm border-gray-50"
              alt="Profile Avatar"
              @error="handleImageError"
            />
            <label
              class="absolute inset-0 flex items-center justify-center transition-opacity rounded-full opacity-0 cursor-pointer bg-black/40 group-hover:opacity-100"
            >
        ...
        <div class="flex-grow space-y-6">
          <div class="flex items-start justify-between">
            ...
            <button
              @click="openInfoModal"
              class="p-2 text-blue-600 transition rounded-full hover:bg-gray-100"
            >
              <svg
          ...
          <div
            class="flex items-start justify-between pt-4 border-t border-gray-50"
          >
            <div>
              <label
                class="block mb-1 text-xs font-bold tracking-widest text-gray-500 uppercase"
    ...
    <div class="flex justify-end pt-6 mt-10 border-t border-gray-100">
      <button
        @click="handleLogout"
        class="px-6 py-2 font-semibold text-white transition duration-200 bg-red-500 shadow-sm hover:bg-red-600 rounded-xl"
      >
        Logout
      </button>
    </div>
    ...
    >
      <div class="w-full max-w-md p-8 bg-white shadow-2xl rounded-3xl">
        <h3 class="mb-6 text-2xl font-bold">Update Info</h3>
        <form @submit.prevent="submitInfoUpdate" class="space-y-4">
          ...
          <div class="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              class="flex-1 py-3 font-bold text-white bg-black rounded-xl"
            >
              Save Info
            </button>
          </div>
    ...
    >
      <div class="w-full max-w-md p-8 bg-white shadow-2xl rounded-3xl">
        <h3 class="mb-6 text-2xl font-bold">Update Password</h3>
        <form @submit.prevent="submitPasswordUpdate" class="space-y-4">
          ...
          <div class="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              class="flex-1 py-3 font-bold text-white bg-blue-600 rounded-xl"
            >
              Update
            </button>
          </div>
    ...
    <div class="pt-12 mt-12 border-t border-gray-100">
      ...
      <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div
          ...
          :key="addr.id"
          class="relative p-6 border border-gray-100 bg-gray-50/50 rounded-xl"
        >
          <div v-if="addr.is_default" class="mb-2 text-xs text-gray-400">
            Default address
          </div>

          <button
            @click="openModal(addr)"
            class="absolute text-blue-500 top-4 right-4 hover:text-blue-700"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="w-4 h-4"
          ...
          <p class="text-sm text-gray-600">{{ addr.details.location }}</p>

          <p class="text-sm text-gray-600">{{ addr.details.type || "-" }}</p>

          <p class="text-sm text-gray-600">{{ addr.details.city }}</p>

          <p class="text-sm text-gray-600">
            {{ addr.details.province }} {{ addr.details.postal_code }}
          </p>

          <p class="text-sm text-gray-600">{{ addr.details.region }}</p>
        </div>
      </div>
    </div>
    ...
    <div
      ...
      class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm"
    >
      <div class="relative w-full max-w-xl p-8 bg-white shadow-2xl rounded-3xl">
        <button
        ...
        <form @submit.prevent="saveAddress" class="space-y-4">
          ...
          <div class="space-y-4">
            <div class="flex flex-col">
              <label class="mb-1 text-xs text-gray-400">Country/region</label>

              <select
                v-model="form.region"
                @change="fetchProvinces"
                class="w-full p-3 border outline-none bg-gray-50 rounded-xl focus:ring-1 focus:ring-blue-500"
              >
            ...
            <div class="grid grid-cols-2 gap-4">
              <input
                ...
                placeholder="First name"
                class="p-3 border outline-none bg-gray-50 rounded-xl"
                required
              />
          ...
          <div class="flex items-center justify-between pt-6">
            ...
            <div class="flex gap-4 ml-auto">
              ...
              <button
                type="submit"
                class="px-8 py-3 font-bold text-white bg-blue-600 rounded-xl"
              >
                Save
              </button>
            </div>
...
<!-- <template>
  <div class="max-w-4xl px-6 py-20 mx-auto">
    ...
    >
      <div
        class="relative w-full max-w-2xl p-8 my-8 bg-white shadow-2xl rounded-3xl"
      >
        <button
          @click="showModal = false"
          class="absolute text-gray-400 top-6 right-6 hover:text-black"
        ...
        <form @submit.prevent="saveAddress" class="space-y-4">
          ...
          <div class="relative">
            <textarea
              ...
              class="w-full p-3 border outline-none resize-none bg-gray-50 rounded-xl"
              required
            ></textarea>
          ...
          <div class="flex items-center justify-between pt-6">
            ...
            <div class="flex gap-4 ml-auto">
              ...
              <button
                type="submit"
                class="px-8 py-3 font-bold text-white bg-blue-600 shadow-md rounded-xl shadow-blue-500/30"
              >
                Save Address
              </button>
            </div>
  ...
  <div class="max-w-4xl px-6 py-20 mx-auto">
    <div class="p-8 bg-white border border-gray-200 shadow-sm rounded-2xl">
      ...
      <div v-if="userData" class="flex flex-col gap-10 md:flex-row">
        <div class="flex flex-col items-center gap-4">
          <div class="relative group">
            <img :key="userData.profile_image" :src="userData.profile_image" class="object-cover w-32 h-32 border-4 rounded-full shadow-sm border-gray-50" alt="Profile Avatar" @error="handleImageError" />
            <label class="absolute inset-0 flex items-center justify-center transition-opacity rounded-full opacity-0 cursor-pointer bg-black/40 group-hover:opacity-100">
              <span class="text-xs font-bold text-white">Change</span>
        ...
        <div class="flex-grow space-y-6">
          <div class="flex items-start justify-between">
            <div>
              <label class="block mb-1 text-xs font-bold tracking-widest text-gray-500 uppercase">Full Name & Email</label>
              <p class="text-xl font-medium">{{ userData.first_name }} {{ userData.last_name }}</p>
              <p class="text-gray-400">{{ userData.email }}</p>
              <p class="text-sm italic text-gray-400">{{ userData.phone || "No phone number added" }}</p>
            </div>
            <button @click="openInfoModal" class="p-2 text-blue-600 transition rounded-full hover:bg-gray-100">
          ...
          <div class="flex items-start justify-between pt-4 border-t border-gray-50">
            <div>
              <label class="block mb-1 text-xs font-bold tracking-widest text-gray-500 uppercase">Your Password</label>
              <p class="text-xl tracking-widest">••••••••</p>
            </div>
            <button @click="showPasswordModal = true" class="p-2 text-blue-600 transition rounded-full hover:bg-gray-100">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
            </button>
          </div>
        </div>
    ...
    <div class="flex justify-end pt-6 mt-10 border-t border-gray-100">
      <button @click="handleLogout" class="px-6 py-2 font-semibold text-white transition duration-200 bg-red-500 shadow-sm hover:bg-red-600 rounded-xl">Logout</button>
    </div>

    <div v-if="showInfoModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
       <div class="w-full max-w-md p-8 bg-white shadow-2xl rounded-3xl">
        <h3 class="mb-6 text-2xl font-bold">Update Info</h3>
        <form @submit.prevent="submitInfoUpdate" class="space-y-4">
          <input v-model="infoForm.first_name" placeholder="First Name" class="w-full p-3 border outline-none bg-gray-50 rounded-xl" required />
          <input v-model="infoForm.last_name" placeholder="Last Name" class="w-full p-3 border outline-none bg-gray-50 rounded-xl" required />
          <input v-model="infoForm.email" type="email" placeholder="Email" class="w-full p-3 border outline-none bg-gray-50 rounded-xl" required />
          <input v-model="infoForm.phone" type="tel" placeholder="Phone Number" class="w-full p-3 border outline-none bg-gray-50 rounded-xl" />
          <div class="flex gap-3 pt-4">
            <button type="button" @click="showInfoModal = false" class="flex-1 text-gray-500">Cancel</button>
            <button type="submit" class="flex-1 py-3 font-bold text-white bg-black rounded-xl">Save Info</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div v-if="showPasswordModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
       <div class="w-full max-w-md p-8 bg-white shadow-2xl rounded-3xl">
        <h3 class="mb-6 text-2xl font-bold">Update Password</h3>
        <form @submit.prevent="submitPasswordUpdate" class="space-y-4">
          <input v-model="passForm.old_password" type="password" placeholder="Old Password" class="w-full p-3 border outline-none bg-gray-50 rounded-xl" required />
          <input v-model="passForm.password" type="password" placeholder="New Password" class="w-full p-3 border outline-none bg-gray-50 rounded-xl" required />
          <input v-model="passForm.password_confirmation" type="password" placeholder="Confirm New Password" class="w-full p-3 border outline-none bg-gray-50 rounded-xl" required />
          <div class="flex gap-3 pt-4">
            <button type="button" @click="showPasswordModal = false" class="flex-1 text-gray-500">Cancel</button>
            <button type="submit" class="flex-1 py-3 font-bold text-white bg-blue-600 rounded-xl">Update</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div class="pt-12 mt-12 border-t border-gray-100">
      ...
      <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div v-for="addr in addresses" :key="addr.id" class="relative p-6 border border-gray-100 bg-gray-50/50 rounded-xl">
          <div v-if="addr.is_default" class="mb-2 text-xs text-gray-400">Default address</div>
          <button @click="openModal(addr)" class="absolute text-blue-500 top-4 right-4 hover:text-blue-700">
          ...
          <p class="text-sm text-gray-600">{{ addr.details.location }}</p>
          <p class="text-sm text-gray-600">{{ addr.details.city }}, {{ addr.details.province }} {{ addr.details.postal_code }}</p>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto bg-black/40 backdrop-blur-sm">
      <div class="relative w-full max-w-2xl p-8 my-8 bg-white shadow-2xl rounded-3xl">
        <button @click="showModal = false" class="absolute text-gray-400 top-6 right-6 hover:text-black">✕</button>
        <h3 class="mb-6 text-2xl font-bold">{{ isEdit ? "Edit address" : "Add address" }}</h3>
        ...
        <form @submit.prevent="saveAddress" class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            <input v-model="form.first_name_address" placeholder="First name" class="p-3 border outline-none bg-gray-50 rounded-xl" required />
            <input v-model="form.last_name_address" placeholder="Last name" class="p-3 border outline-none bg-gray-50 rounded-xl" required />
          </div>

          <div class="grid grid-cols-2 gap-4">
            <select v-model="form.province" class="p-3 border outline-none bg-gray-50 rounded-xl" required>
              <option value="" disabled>Select Province</option>
              <option v-for="p in filteredProvinces" :key="p" :value="p">{{ p }}</option>
            </select>
            <input v-model="form.city" placeholder="City" class="p-3 border outline-none bg-gray-50 rounded-xl" required />
          </div>

          <div class="mt-4 overflow-hidden border border-gray-200 rounded-xl">
            <div class="flex items-center justify-between gap-2 p-3 border-b border-gray-200 bg-gray-50">
              <p class="hidden text-xs font-bold tracking-widest text-gray-500 uppercase sm:block">Pin Location</p>
          ...
          <div class="relative">
            <textarea v-model="form.address_location" rows="2" placeholder="Full street address (auto-filled from map)" class="w-full p-3 border outline-none resize-none bg-gray-50 rounded-xl" required></textarea>
            <p class="text-[10px] text-gray-400 mt-1">*Drag pin or search location to auto-fill address.</p>
          </div>

          <div class="grid grid-cols-2 gap-4">
            <input v-model="form.location_type" placeholder="Apartment, suite (optional)" class="p-3 border outline-none bg-gray-50 rounded-xl" />
            <input v-model="form.postal_code" placeholder="Postal code" class="p-3 border outline-none bg-gray-50 rounded-xl" required />
          </div>
          ...
          <div class="flex items-center justify-between pt-6">
            ...
            <div class="flex gap-4 ml-auto">
              ...
              <button type="submit" class="px-8 py-3 font-bold text-white bg-blue-600 shadow-md rounded-xl shadow-blue-500/30">Save Address</button>
            </div>
          </div>
        </form>
      </div>
      ...
      <div v-if="userData" class="flex flex-col gap-10 md:flex-row">
        ...
        <div class="flex-grow space-y-6">
          ...
          <div class="pt-8 mt-8 border-t border-gray-100">
            <router-link
              to="/favorites"
              class="flex items-center justify-between p-6 transition-all border border-gray-100 cursor-pointer group bg-gray-50 hover:bg-red-50 rounded-3xl hover:border-red-100"
            >
              <div class="flex items-center gap-5">
                <div
                  class="flex items-center justify-center w-12 h-12 text-red-500 transition-colors bg-white rounded-full shadow-sm group-hover:bg-red-100"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="w-6 h-6"
                ...
                <div>
                  <h2
                    class="text-xl font-bold text-gray-900 transition-colors group-hover:text-red-600"
                  >
                    My Favorites
                  </h2>
              ...
              <div
                class="text-gray-400 transition-all group-hover:text-red-500 group-hover:translate-x-2"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="w-6 h-6"
          ...
          <div class="pt-6 mt-6 border-t border-gray-50">
            <div
              class="relative p-6 overflow-hidden text-white shadow-lg bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl"
            >
              <svg
                class="absolute w-24 h-24 text-yellow-500 -right-4 -top-4 opacity-20"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
              ...
              <div class="relative z-10 flex items-center justify-between">
                <div>
                  <h3
                    class="text-lg font-bold tracking-wide text-yellow-400 uppercase"
                  >
                    Solher Club Member
                  </h3>
                ...
                <div class="flex flex-col items-end">
                  <div
                    ...
                    "
                    class="px-4 py-2 text-xs font-bold tracking-widest uppercase rounded-full shadow-inner"
                  >
...
<!-- <template>
  <div class="max-w-4xl px-6 py-20 mx-auto">
    ...
    <div class="pt-12 mt-12 border-t border-gray-100">
      ...
      <div v-if="isLoadingAddresses" class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div v-for="i in 2" :key="i" class="p-6 space-y-3 border border-gray-100 bg-gray-50/50 rounded-xl animate-pulse">
          ...
          <div class="w-32 h-4 mt-4 bg-gray-300 rounded"></div>
        </div>
      </div>

      <div v-else-if="addresses.length > 0" class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div
          v-for="addr in addresses"
          :key="addr.id"
          class="relative p-6 border border-gray-100 bg-gray-50/50 rounded-xl"
      ...
      <div v-else class="py-10 text-center border border-gray-300 border-dashed bg-gray-50 rounded-xl">
        <p class="text-sm italic text-gray-500">No saved addresses.</p>
      </div>
      ...
      <div class="space-y-6 lg:col-span-4">
        ...
        <template v-if="userData">
          ...
          <button
            ...
            class="flex items-center justify-between w-full p-5 mt-4 text-left transition-all border border-gray-800 shadow-lg bg-gradient-to-r from-gray-900 to-black group hover:shadow-xl rounded-2xl"
          >
            <div class="flex items-center gap-4">
              <div
...
// State loading khusus untuk alamat
const isLoadingAddresses = ref(true);

const form = ref({
  id: null,
  region: "Indonesia", // Default Country
  first_name_address: "",
  last_name_address: "",
  address_location: "",
  location_type: "",
...
// [PERBAIKAN] Logika dinamis untuk menarik data Provinsi berdasarkan Negara yang dipilih
const fetchProvinces = () => {
  const selectedCountry = countries.value.find((c) => c.name === form.value.region);
  if (selectedCountry) {
...
const openModal = (data = null) => {
  isEdit.value = !!data;
  if (data) {
    // [PERBAIKAN] Pastikan properti region ditarik dari data yang tersimpan
    ...
    // Muat data provinsi untuk negara yang dipilih saat diedit
    fetchProvinces();
  } else {
    form.value = {
      region: "Indonesia",
    ...
    center.value = [-7.250445, 112.768845];
    markerLatLng.value = [-7.250445, 112.768845];
    fetchProvinces(); // Load provinsi untuk Indonesia
  }
  showModal.value = true;
};
...
<!-- <template>
  <div class="max-w-6xl px-4 py-12 mx-auto sm:px-6 lg:py-16 animate-fade-in">
    ...
    >
      <div
        class="absolute top-0 right-0 w-64 h-64 bg-blue-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse"
      ></div>
      ...
      >
        <div class="flex-1">
          <div class="flex items-center gap-3 mb-2">
            <h3 class="text-lg font-bold text-white">Sempurnakan Profil Anda</h3>
            <span
              class="px-2.5 py-0.5 text-xs font-black text-blue-900 bg-blue-100 rounded-full"
              >{{ profileCompleteness }}%</span
            >
          </div>
          <p class="text-sm text-blue-200">
            Dapatkan pengalaman belanja yang lebih cepat dan aman. Anda belum melengkapi:
        ...
        <div class="w-full md:w-1/3">
          <div
            class="w-full h-3 bg-blue-950 rounded-full overflow-hidden border border-blue-900/50"
          >
            <div
              class="h-full bg-gradient-to-r from-blue-400 to-cyan-300 rounded-full transition-all duration-1000 ease-out relative"
          ...
          <p
            class="text-[10px] text-right text-blue-300 mt-1.5 font-medium tracking-widest uppercase"
          >
            {{ 100 - profileCompleteness }}% Lagi menuju Sempurna
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="userData" class="grid grid-cols-1 gap-8 lg:grid-cols-12">
      <div class="space-y-6 lg:col-span-4">
        ...
        >
          ...
          <div class="px-8 pb-8 -mt-12 text-center relative z-10">
              ...
              >
                ...
                <div class="flex-1">
                  ...
                  <p class="font-medium text-gray-900">
                    ...
                    <span
                      v-if="!userData.phone"
                      class="text-[10px] text-red-500 italic font-normal ml-1"
                      >(Belum diisi)</span
                    >
                  </p>
                </div>
            ...
            <div class="flex justify-center gap-3 mt-6">
              <button
                @click="openInfoModal"
                class="flex-1 flex items-center justify-center gap-2 h-10 text-xs font-bold text-gray-700 transition-colors bg-white border border-gray-200 rounded-xl hover:bg-gray-50 hover:text-black"
              >
                Edit Profil
              </button>
              <button
                @click="showPasswordModal = true"
                class="flex items-center justify-center w-10 h-10 text-gray-500 transition-colors bg-white border border-gray-200 rounded-xl hover:bg-gray-50"
                title="Change Password"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
              ...
              <button
                @click="handleLogout"
                class="flex items-center justify-center w-10 h-10 text-red-500 transition-colors bg-red-50 border border-red-100 rounded-xl hover:bg-red-100"
                title="Logout"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
        ...
        >
          <svg
            class="absolute w-40 h-40 opacity-10 -right-8 -top-8 text-white mix-blend-overlay"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
          ...
          <div class="relative z-10">
            <div class="flex items-start justify-between">
              <div>
                <h3 class="text-sm font-black tracking-widest uppercase opacity-90">
                  {{ $t("profile.solher_club") }}
                </h3>
                <div
                  class="inline-flex items-center gap-1.5 px-3 py-1 mt-2 text-[10px] font-black tracking-widest uppercase rounded-full shadow-inner bg-white/20 backdrop-blur-sm border border-white/20"
                >
            ...
            <div class="mt-8">
              <template v-if="userData.is_membership">
                <p
                  class="text-[10px] uppercase tracking-widest mb-1 opacity-80 font-medium"
                >
                  {{ $t("profile.available_points") }}
                </p>
                <div class="flex items-baseline gap-1">
                ...
                <div v-if="userTier.nextTierTarget" class="mt-4">
                  <div
                    class="flex justify-between text-[9px] font-bold tracking-widest uppercase opacity-80 mb-1"
                  >
                  ...
                  <div class="h-1.5 w-full bg-black/20 rounded-full overflow-hidden">
                    <div
                      ...
                      :style="{
                        width: `${Math.min(
                          (userData.point / userTier.nextTierTarget) * 100,
                          100
                        )}%`,
                      }"
                    ></div>
              ...
              <template v-else>
                <p class="text-xs leading-relaxed opacity-90 mt-2">
                  Tingkatkan total belanja Anda hingga <strong>Rp 100.000</strong> untuk
                  membuka fitur eksklusif, cashback poin, dan promo khusus member.
                </p>
              </template>
            </div>
          </div>
        ...
        <router-link
          to="/favorites"
          class="flex items-center justify-between p-5 transition-all bg-white border border-gray-100 shadow-sm group hover:border-red-200 hover:shadow-md rounded-2xl"
        >
          <div class="flex items-center gap-4">
            <div
              class="flex items-center justify-center w-10 h-10 text-red-500 transition-colors bg-red-50 rounded-xl group-hover:bg-red-100"
        ...
        <template v-if="userData">
          <router-link
            v-if="userData.is_affiliate"
            to="/affiliate-demo"
            class="flex items-center justify-between p-5 mt-4 transition-all bg-white border border-gray-100 shadow-sm group hover:border-blue-200 hover:shadow-md rounded-2xl"
          >
            <div class="flex items-center gap-4">
              <div
                class="flex items-center justify-center w-10 h-10 text-blue-600 transition-colors bg-blue-50 rounded-xl group-hover:bg-blue-100"
          ...
          <button
            v-else
            @click="showAffiliateModal = true"
            class="flex items-center justify-between w-full p-5 mt-4 text-left transition-all border shadow-lg bg-gradient-to-r from-gray-900 to-black border-gray-800 group hover:shadow-xl hover:border-yellow-500/50 rounded-2xl"
          >
            <div class="flex items-center gap-4">
              <div
                class="flex items-center justify-center w-10 h-10 text-black transition-colors bg-yellow-400 rounded-xl shadow-[0_0_15px_rgba(250,204,21,0.4)]"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="w-5 h-5"
      ...
      <div class="lg:col-span-8">
        ...
        >
          <div
            class="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-6 mb-6 border-b border-gray-100 gap-4"
          >
            <div>
              <h2 class="text-xl font-bold text-gray-900">
                {{ $t("profile.shipping_addresses") }}
          ...
          <div v-if="isLoadingAddresses" class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div
              v-for="i in 4"
              :key="i"
              class="p-6 border border-gray-100 bg-gray-50/50 rounded-2xl animate-pulse"
            >
          ...
          >
            ...
            >
              <div
                v-if="addr.is_default"
                class="absolute px-2.5 py-1 text-[9px] font-black tracking-widest text-blue-700 uppercase bg-blue-100 rounded-full top-5 right-5 shadow-sm"
              >
                {{ $t("profile.default") }}
              </div>
              ...
              <div class="flex items-center gap-3 mb-4">
                <div
                  class="flex items-center justify-center w-10 h-10 text-blue-500 bg-blue-50 rounded-full"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="w-5 h-5"
              ...
              <div class="space-y-1.5 text-xs text-gray-600 leading-relaxed">
                ...
                <div
                  v-if="addr.details.latitude && addr.details.longitude"
                  class="inline-flex items-center gap-1 px-2.5 py-1 mt-2 text-[9px] font-bold tracking-widest text-green-700 uppercase bg-green-50 rounded-md border border-green-100"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="w-3 h-3"
          ...
          <div
            v-else
            class="flex flex-col items-center justify-center py-20 text-center border-2 border-gray-200 border-dashed rounded-3xl bg-gray-50/50"
          >
            <div
            ...
            <button
              @click="openModal()"
              class="px-6 py-2.5 mt-6 text-xs font-bold text-white bg-black rounded-full hover:bg-gray-800 shadow-md transition-all hover:scale-105"
            >
              {{ $t("profile.add_first_address") }}
            </button>
          </div>
          ...
          <form @submit.prevent="saveAddress" class="space-y-5">
            ...
            <div class="mb-4">
              ...
              <select
                v-model="form.region"
                @change="fetchProvinces"
                class="w-full px-4 py-3 text-sm transition-colors border border-gray-200 outline-none bg-gray-50 rounded-xl focus:border-blue-500 focus:bg-white"
                required
              >
                <option v-for="c in countries" :key="c.isoCode" :value="c.name">
                  {{ c.name }}
            ...
            <div>
              ...
              <textarea
                ...
                rows="3"
                placeholder="Street name, building, house number..."
                class="w-full px-4 py-3 text-sm transition-colors border border-gray-200 resize-none bg-gray-50 rounded-xl focus:border-blue-500 focus:bg-white outline-none custom-scrollbar"
                required
              ></textarea>
            </div>
      ...
      <div class="w-full max-w-md p-8 bg-white shadow-2xl rounded-3xl animate-fade-in">
        ...
        <form @submit.prevent="submitAffiliateApplication" class="space-y-4">
          ...
          <button
            type="submit"
            :disabled="isSubmittingAffiliate"
            class="w-full py-3.5 mt-4 font-bold tracking-widest uppercase text-white transition-colors bg-gray-900 rounded-xl hover:bg-black disabled:bg-gray-400"
          >
            {{ isSubmittingAffiliate ? "Mengirim..." : "Kirim Pendaftaran" }}
          </button>
        </form>
...
// 👇 COMPUTED PROPERTIES UNTUK GAMIFIKASI & TIER LEVEL 👇
...
// 👇 1. INISIALISASI SINKRON LANGSUNG DARI LOCAL STORAGE (ANTI-FOUC) 👇
...
// COMPUTED PROPERTIES UNTUK GAMIFIKASI & TIER LEVEL
const profileCompleteness = computed(() => {
  if (!userData.value) return 100; // Default 100 saat menunggu data agar tidak memicu flash
  let score = 0;
  let total = 6;
...
const fetchAddresses = async () => {
  try {
    ...
    addresses.value = res.data.data;
    // Simpan ke localStorage agar kunjungan berikutnya instan
  ...
  <div class="max-w-6xl px-4 py-12 mx-auto sm:px-6 lg:py-16 animate-fade-in">
    ...
    <div v-if="userData" class="grid grid-cols-1 gap-8 lg:grid-cols-12">
      <div class="space-y-6 lg:col-span-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <router-link to="/orderpage" class="flex flex-col items-start p-4 transition-all bg-white border border-gray-200 shadow-sm group hover:border-black hover:shadow-md rounded-2xl">
            ...
            <div class="flex flex-col sm:flex-row items-center sm:items-end gap-5 -mt-16 relative z-10 mb-6">
              <div class="relative inline-block group shrink-0">
                <img
                  :key="userData.profile_image || 'default'"
                  :src="userData.profile_image || defaultProfile"
                  class="relative object-cover w-32 h-32 border-4 border-white rounded-full shadow-lg bg-white transition-transform duration-500 group-hover:scale-105"
                  alt="Profile Avatar"
                  @error="handleImageError"
                />
        ...
        <div class="p-6 sm:p-10 bg-white border border-gray-100 shadow-xl shadow-gray-200/40 rounded-[2rem] min-h-[600px] flex flex-col">
          ...
          <div v-else class="grid grid-cols-1 gap-5 md:grid-cols-2 content-start flex-grow">
            ...
            >
              ...
              <button
                @click="openModal(addr)"
                class="absolute p-2.5 text-gray-400 transition-all bg-gray-100 border border-gray-200 rounded-xl opacity-100 sm:opacity-0 top-4 right-4 group-hover:opacity-100 hover:text-blue-600 hover:bg-blue-50 hover:border-blue-200 shadow-sm"
...
const fetchAddresses = async () => {
  try {
    ...
    localStorage.setItem("cached_addresses", JSON.stringify(res.data));
  } catch (error) {
    console.error("Failed to fetch addresses");
  } finally {
    isLoadingAddresses.value = false;
