      ...
      <div class="flex items-center gap-4 w-full md:w-auto">
        <div class="relative w-full md:w-64">
          ...
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Search No. Trans or Name..."
      ...
      <table class="w-full text-left border-collapse min-w-[900px]">
        ...
        <tbody class="text-gray-700 text-sm">
          ...
          >
            ...
            <td class="p-4">
              <span class="bg-gray-100 px-2 py-1 rounded text-xs font-mono">{{
                payment.debit_coa?.coa_no
              }}</span>
              <p class="text-xs text-gray-500 mt-1 truncate w-32">
                {{ payment.debit_coa?.name }}
              </p>
            </td>
            <td class="p-4">
              <span class="bg-gray-100 px-2 py-1 rounded text-xs font-mono">{{
                payment.kredit_coa?.coa_no
              }}</span>
              <p class="text-xs text-gray-500 mt-1 truncate w-32">
                {{ payment.kredit_coa?.name }}
              </p>
            </td>
          ...
          <tr v-if="paginatedPayments.length === 0">
            <td colspan="6" class="text-center py-16 text-gray-400 italic">
              No records found.
            </td>
          </tr>
        </tbody>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                v-model="form.type"
                class="bg-gray-50 p-3 rounded-xl outline-none focus:ring-2 focus:ring-black w-full font-bold"
                required
              >
          ...
          <div
            class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50/50 p-4 border rounded-xl"
          >
            <div>
              <label
                class="block mb-1 font-bold text-xs uppercase text-gray-500 tracking-widest"
                >Debit Account (To)</label
              >
              <select
                v-model="form.debit_coa_id"
                class="bg-white p-3 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-black w-full"
                required
              >
          ...
          <div>
            ...
            <input
              v-model="form.amount"
              type="number"
              class="bg-gray-50 p-3 rounded-xl outline-none focus:ring-2 focus:ring-black w-full font-mono text-xl text-black font-black"
              required
            />
          </div>
...
const showingEnd = computed(() =>
  Math.min(currentPage.value * itemsPerPage.value, filteredPayments.value.length)
);
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await axios.put(
        `${BASE_URL}/admin/payments/${form.value.id}`,
        payload,
        axiosConfig
      );
    } else {
