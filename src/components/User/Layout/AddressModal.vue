    ...
    <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm sm:p-6">
      <div class="relative w-full max-w-2xl bg-white shadow-2xl rounded-3xl animate-fade-in flex flex-col max-h-[90vh] md:max-h-[85vh]">
          ...
          <form @submit.prevent="saveAddress" class="space-y-5">
            ...
            <div class="relative overflow-hidden border border-gray-200 rounded-2xl">
              <div class="flex items-start gap-2 px-4 py-3 bg-amber-50 border-b border-amber-100">
                <p class="text-[11px] text-amber-800 leading-relaxed"><span class="font-bold">{{ $t("profile.pin_location") }}</span> Ensure the pin matches your exact delivery point.</p>
              </div>
              <div class="flex flex-col sm:flex-row items-center justify-between gap-2 p-3 border-b border-gray-200 bg-gray-50">
                <div class="relative w-full sm:flex-1">
                  <input type="text" v-model="searchQuery" @input="handleSearchInput" placeholder="Search area..." class="w-full px-3 py-2.5 text-xs transition-colors border border-gray-300 outline-none rounded-xl focus:ring-1 focus:ring-blue-500 focus:border-blue-500" />
                  <div v-if="searchResults.length > 0" class="absolute z-[999] mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg max-h-40 overflow-y-auto custom-scrollbar">
                    <div v-for="(result, idx) in searchResults" :key="idx" @click="selectSearchResult(result)" class="px-3 py-2.5 text-xs text-gray-700 border-b cursor-pointer hover:bg-blue-50 last:border-0">{{ result.display_name }}</div>
                  </div>
                </div>
            ...
            <div>
              ...
              <textarea v-model="form.address_location" rows="3" placeholder="Street name, building..." class="w-full px-4 py-3 text-sm transition-colors border border-gray-200 resize-none bg-gray-50 rounded-xl focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none custom-scrollbar" required></textarea>
            </div>
        ...
        <div class="flex items-center justify-end gap-3 p-6 border-t border-gray-100 shrink-0 md:p-8 md:pt-5 bg-gray-50/50">
          <button type="button" @click="$emit('close')" class="px-5 py-2.5 text-sm font-bold text-gray-600 transition-colors bg-white border border-gray-300 rounded-xl hover:bg-gray-50 hidden sm:block">{{ $t("profile.cancel") }}</button>
          <button type="button" @click="saveAddress" :disabled="isSubmitting" class="px-6 py-2.5 text-sm font-bold text-white transition-colors bg-blue-600 rounded-xl hover:bg-blue-700 shadow-md shadow-blue-500/20 disabled:opacity-50">
...
watch(() => props.isOpen, (newVal) => {
  if (newVal) {
    ...
    fetchProvinces();
    setTimeout(() => { if (map.value?.leafletObject) map.value.leafletObject.invalidateSize(); }, 300);
  }
});
...
const selectSearchResult = (result) => {
  ...
  if (map.value?.leafletObject) map.value.leafletObject.flyTo([lat, lng], 16);
...
const reverseGeocode = async (lat, lng) => {
  try {
    ...
    if (res.data?.display_name) {
      ...
      if (res.data.address?.postcode) form.value.postal_code = res.data.address.postcode;
    }
  } catch (e) {}
};
...
const saveAddress = async () => {
  ...
  try {
    ...
    emit('saved', res.data.id); // Lemparkan ID alamat baru ke PaymentPage
  } catch (e) {
