<template>
  <div v-if="msg" class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between mb-4">
      <button
        @click="$router.back()"
        class="text-sm font-bold text-gray-500 hover:text-black transition"
      >
        ← Back to Messages
      </button>
    </div>
      ...
      <form v-else @submit.prevent="sendResponse">
        ...
        <div class="mt-4 flex justify-end">
          <button
            type="submit"
            :disabled="isSending"
            class="bg-black hover:bg-gray-800 disabled:bg-gray-400 text-white px-8 py-3 rounded-xl font-bold uppercase tracking-widest text-xs transition"
          >
