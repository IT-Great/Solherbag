    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-10">
      ...
      <div class="p-6 bg-purple-50/50 rounded-2xl border border-purple-100">
        ...
        <p class="text-4xl font-black text-purple-700">
          ...
          <span class="text-sm font-bold text-purple-400">/ {{ finalSaleCount }}</span>
        </p>
      </div>
    </div>
          ...
          >
            ...
            <td class="py-4 font-bold text-gray-800">
              <div class="flex items-center gap-2">
                {{ p.name }}
                <span
                  v-if="p.is_final_sale"
                  class="bg-red-700 text-white px-1.5 py-0.5 rounded text-[8px] font-black tracking-widest uppercase shadow-sm whitespace-nowrap"
                  title="This product is non-refundable"
                >
                  Final Sale
                </span>
              </div>
...
// 👇 [PERBAIKAN] Menghitung total produk Final Sale 👇
const finalSaleCount = computed(() => {
  return products.value.filter((p) => p.is_final_sale).length;
});
...
<template>
  ...
  >
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-10">
      ...
      <!-- 👇 [PERBAIKAN] Tampilkan juga statistik Final Sale 👇 -->
      <div class="p-6 bg-purple-50/50 rounded-2xl border border-purple-100">
        <p class="text-xs text-purple-600 font-bold uppercase tracking-widest mb-1">
          Sale / Final Sale
        </p>
    ...
    >
      ...
      <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
        ...
        <div class="flex items-center gap-2">
          <button
            @click="exportToExcel"
            class="flex items-center gap-2 px-4 py-2 text-xs font-bold text-green-700 bg-green-50 border border-green-200 rounded-xl hover:bg-green-100 transition h-[42px]"
          >
          ...
          <button
            @click="exportToPDF"
            class="flex items-center gap-2 px-4 py-2 text-xs font-bold text-red-700 bg-red-50 border border-red-200 rounded-xl hover:bg-red-100 transition h-[42px]"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
              <polyline points="14 2 14 8 20 8"></polyline>
...
// 👇 FUNGSI EXPORT DATA (Menggunakan filteredProducts agar hasil pencarian ikut diunduh) 👇
const getExportData = () => {
  return filteredProducts.value.map((p) => {
    const price = parseFloat(p.price) || 0;
    const discountPrice = parseFloat(p.discount_price);
    ...
    return {
      ...
      "Base Price (IDR)": price,
      "Discount Price (IDR)": isDiscounted ? discountPrice : "-",
      "Stock": p.stock,
      "Final Sale": p.is_final_sale ? "Yes" : "No"
    };
  });
};
...
const exportToExcel = () => {
  ...
  XLSX.writeFile(
    workbook,
    `Solher_Products_${new Date().toISOString().split("T")[0]}.xlsx`
  );
};
...
const exportToPDF = () => {
  ...
  doc.setFontSize(16);
  doc.text("Solher - Products Master Data", 14, 15);
  doc.setFontSize(10);
  doc.text(`Generated on: ${new Date().toLocaleString()}`, 14, 22);

  const columns = ["Code", "Product Name", "Category", "Base Price", "Disc. Price", "Stock", "Final Sale"];
  const rows = data.map((item) => [
    item["Code"],
    item["Product Name"],
    item["Category"],
    item["Base Price (IDR)"].toLocaleString('id-ID'),
    item["Discount Price (IDR)"] !== "-" ? item["Discount Price (IDR)"].toLocaleString('id-ID') : "-",
    item["Stock"],
    item["Final Sale"],
  ]);
  ...
  doc.save(`Solher_Products_${new Date().toISOString().split("T")[0]}.pdf`);
};
