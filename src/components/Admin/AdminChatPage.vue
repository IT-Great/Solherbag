  ...
  <div class="flex flex-col w-full max-w-4xl mx-auto bg-[#F9FAFB] border border-gray-200 shadow-sm rounded-2xl overflow-hidden animate-fade-in mt-4 mb-8 h-[calc(100dvh-130px)]">
    ...
    <div class="relative z-10 flex items-center px-6 py-4 bg-white border-b border-gray-200 shadow-sm shrink-0">
      ...
      <div v-if="receiverProfile" class="flex items-center gap-3">
        ...
        <div>
          ...
          <p v-else class="text-[10px] text-gray-500 font-bold uppercase tracking-widest flex items-center gap-1.5 mt-0.5">
    ...
    <div class="flex flex-col p-6 overflow-y-auto grow custom-scrollbar" ref="chatContainer">
      ...
      <div v-for="msg in messages" :key="msg.id" class="flex w-full mb-4" :class="msg.sender_id === myId ? 'justify-end' : 'justify-start'">
        <div :class="msg.sender_id === myId ? 'bg-black text-white rounded-l-2xl rounded-tr-2xl' : 'bg-white border border-gray-200 text-black rounded-r-2xl rounded-tl-2xl'" class="px-5 py-3 max-w-[75%] shadow-sm relative group min-w-[100px]">
...
// [PERHATIAN KHUSUS ADMIN] Pengambilan Data dan Token menggunakan kunci Admin
const adminData = JSON.parse(localStorage.getItem("admin"));
const myId = ref(adminData ? Number(adminData.id) : null);
const token = localStorage.getItem("admin_token");
...
// 👇 PERBAIKAN: Fungsi Mengirim Sinyal Mengetik ke Pelanggan
const handleTyping = () => {
  const now = Date.now();
  if (now - lastTypingTime > 1500) {
    lastTypingTime = now;
...
const setupPusher = () => {
  ...
  window.Echo.private(`chat.${supportId.value}`)
    .listen(".message.sent", (e) => {
      const incomingMsg = e.message || e;
      if (
        incomingMsg.sender_id === receiverId ||
        incomingMsg.receiver_id === receiverId
      ) {
    ...
    .listen(".user.typing", (e) => {
      // Tangkap sinyal saat pelanggan sedang mengetik
      if (e.typer_id === receiverId) {
        isOpponentTyping.value = true;
        clearTimeout(typingTimeout);
...
// 👇 PERBAIKAN: Fungsi Mengubah Status Pesan Menjadi 'Sudah Dibaca'
const markAsRead = async () => {
  if (!token) return;
  try {
    await axios.post(
    ...
    // 👇 PENTING: Sinyal ini yang akan menghapus badge dari Sidebar & UserListPage
  ...
  window.Echo.private(`chat.${supportId.value}`)
    .listen(".message.sent", (e) => {
      ...
      if (
        ...
        // Tandai terbaca secara realtime jika Admin sedang melihat layar ini
        if (incomingMsg.sender_id === receiverId) {
          markAsRead();
        }
      }
    })
...
const fetchMessages = async () => {
  ...
  try {
    ...
    // 👇 PANGGIL FUNGSI: Langsung eksekusi begitu Admin membuka jendela chat
    markAsRead();
  } catch (error) {
    console.error("Gagal", error);
  } finally {
