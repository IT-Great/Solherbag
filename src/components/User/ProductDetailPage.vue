...
const handleAction = async (type) => {
  ...
  if (type === "cart") {
    ...
    try {
      ...
      // Ambil ID Cart asli dari database
      const realCartId = response.data.id || response.data.cart_id || response.data.data?.id;
      ...
      if (productImage && cartIcon) {
        ...
        flyer.addEventListener(
          ...
          () => {
            flyer.remove();
            // [PENTING] Kirim object berisi product DAN cartId ke Header/useCart
            window.dispatchEvent(
              new CustomEvent("optimistic-add-to-cart", {
                detail: { product: product.value, cartId: realCartId },
              })
    ...
    return; // STOP! Biarkan Header yang urus databasenya.
  }

  // --- LOGIKA BUY IT NOW (Tetap sama karena beda alur) ---
  try {
    if (type === "buy") {
      Swal.fire({
        title: "Preparing Order...",
