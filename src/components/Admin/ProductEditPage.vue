    ...
    <form @submit.prevent="handleSubmit" class="gap-6 grid grid-cols-1 md:grid-cols-2">
      <div class="space-y-6">
        ...
        <div class="gap-4 grid grid-cols-2">
          <div>
            <label class="block mb-1 font-bold text-sm">Original Price</label>
            <input v-model="form.price" type="number" class="bg-gray-100 p-3 rounded-xl w-full" required />
          </div>
          <div>
            <label class="block mb-1 font-bold text-sm">Discount Price (Optional)</label>
            <input v-model="form.discount_price" type="number" placeholder="Set if on sale" class="bg-gray-100 p-3 rounded-xl w-full" />
          </div>
        </div>
        ...
        <div class="p-4 border border-gray-200 rounded-2xl bg-gray-50/50 space-y-6">
          
          <div>
            <label class="block mb-1 font-bold text-sm">Main Image</label>
            <p class="text-[10px] text-gray-500 mb-2">Leave blank to keep current image.</p>
            <div class="flex items-center gap-4">
              <img v-if="currentImage" :src="currentImage" class="w-16 h-16 object-cover rounded-lg border shadow-sm" alt="Current Main Image">
              <div v-else class="w-16 h-16 bg-gray-200 rounded-lg flex justify-center items-center text-[10px] text-gray-400">No Img</div>
              
              <input type="file" @change="handleFile" accept="image/*" class="w-full text-sm flex-1" />
            </div>
          </div>

          <div class="border-t border-gray-200 pt-4">
            <label class="block mb-1 font-bold text-sm">Variant Images (Max 5)</label>
            <p class="text-[10px] text-gray-500 mb-2">Upload new files to REPLACE all current variant images.</p>
            
            <input type="file" @change="handleVariantImages" accept="image/*" multiple class="w-full text-sm mb-3" />
            
            <div v-if="currentVariantImages && currentVariantImages.length > 0" class="mt-2">
              <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Current Variants:</p>
              <div class="flex gap-2 overflow-x-auto pb-2">
                <img v-for="(img, idx) in currentVariantImages" :key="idx" :src="img" class="w-12 h-12 object-cover rounded-lg border border-gray-300 shadow-sm shrink-0" />
              </div>
            </div>
          </div>

          <div class="border-t border-gray-200 pt-4">
            <label class="block mb-1 font-bold text-sm">Product Video (Max 5MB)</label>
            <p class="text-[10px] text-gray-500 mb-2">Upload new video to REPLACE the current one.</p>
            
            <input type="file" @change="handleVideo" accept="video/mp4,video/quicktime" class="w-full text-sm mb-3" />
            
            <div v-if="currentVideo" class="mt-2 flex items-center gap-2 bg-blue-50 text-blue-700 px-3 py-2 rounded-lg text-xs font-medium border border-blue-100 w-fit">
      ...
      <div class="space-y-4">
        <div>
          <label class="block mb-1 font-bold text-sm">Description</label>
          <textarea v-model="form.description" class="bg-gray-100 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 w-full h-32"></textarea>
        </div>
        <div>
          <label class="block mb-1 font-bold text-sm">Care Instructions</label>
          <textarea v-model="form.care" class="bg-gray-100 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 w-full h-32"></textarea>
        </div>
        <div>
          <label class="block mb-1 font-bold text-sm">Design Details</label>
          <textarea v-model="form.design" class="bg-gray-100 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 w-full h-32"></textarea>
        </div>
      </div>
