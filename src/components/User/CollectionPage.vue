...
<template>
  <div class="relative bg-[#F3F4F6] px-6 md:px-12 pt-12 pb-24 min-h-screen">
    ...
    <div class="mx-auto mb-10 max-w-7xl">
      ...
      <!-- 👇 PERBAIKAN: AREA FILTER & PENCARIAN 👇 -->
      <div class="flex flex-col gap-4 mb-12">
        <!-- Baris Atas: Pencarian & Dropdown Utama -->
        <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div class="relative w-full md:w-80 shrink-0">
            ...
            <input
              v-model="searchQuery"
              type="text"
              :placeholder="$t('collection.search_placeholder', 'Search products...')"
              class="w-full py-3 pl-10 pr-4 text-sm transition bg-white border border-gray-200 rounded-full shadow-sm outline-none focus:border-black focus:ring-1 focus:ring-black"
            />
          </div>

          <div class="flex flex-wrap items-center gap-3 md:flex-nowrap">
            <!-- Dropdown Tipe Tas (Bag Categories) -->
            <div class="relative flex-1 min-w-[160px] md:flex-none">
              <select
              ...
              >
                <option value="">
                  {{ $t("collection.all_bag_types", "All Bag Types") }}
                </option>
            ...
            <!-- Dropdown Collections (Categories excl C005) -->
            <div class="relative flex-1 min-w-[160px] md:flex-none">
              <select
                v-model="selectedCategory"
                class="w-full px-5 py-3 text-[10px] sm:text-xs font-bold tracking-widest text-gray-600 uppercase transition bg-white border border-gray-200 rounded-full shadow-sm outline-none appearance-none cursor-pointer focus:border-black focus:ring-1 focus:ring-black pr-8 truncate hover:bg-gray-50"
              >
                <option value="">
                  {{ $t("collection.all_collections", "All Collections") }}
                </option>
                <option
        ...
        <!-- Baris Bawah: Quick Filter Toggles -->
        <div class="flex flex-wrap items-center gap-2 mt-2">
          <button
            @click="setSpecialFilter('new')"
            :class="
              activeSpecialFilter === 'new'
                ? 'bg-black text-white shadow-md transform scale-105 border-black'
                : 'bg-white text-gray-700 hover:bg-gray-50 border-gray-200'
            "
            class="px-5 py-2 text-[10px] sm:text-xs font-bold tracking-widest uppercase transition-all duration-300 border rounded-full whitespace-nowrap text-center flex items-center gap-1.5"
          >
            <span v-if="activeSpecialFilter === 'new'">✓</span>
            {{ $t("collection.new_arrivals", "New Arrivals") }}
          </button>

          <button
            @click="setSpecialFilter('sale')"
            :class="
              activeSpecialFilter === 'sale'
                ? 'bg-red-600 text-white border-red-600 shadow-md transform scale-105'
                : 'bg-white text-red-500 hover:bg-red-50 border-red-100'
            "
            class="px-5 py-2 text-[10px] sm:text-xs font-bold tracking-widest uppercase transition-all duration-300 border rounded-full whitespace-nowrap text-center flex items-center gap-1.5"
          >
            <span v-if="activeSpecialFilter === 'sale'">✓</span>
            {{ $t("collection.final_sale", "Final Sale") }}
          </button>
          ...
          <button
            ...
            class="px-5 py-2 text-[10px] sm:text-xs font-bold tracking-widest uppercase transition-all duration-300 border rounded-full whitespace-nowrap text-center flex items-center gap-1.5"
          >
            <span v-if="isAccessoriesActive">✓</span>
          ...
          <button
            ...
            @click="resetAllFilters"
            class="px-4 py-2 text-[10px] sm:text-xs font-bold tracking-widest text-gray-400 uppercase transition-all hover:text-black underline ml-auto"
          >
            Clear Filters
          </button>
        </div>
...
// 👇 PERBAIKAN CRO: FILTER STATE 👇
const searchQuery = ref("");
const selectedCategory = ref("");
const selectedBagCategory = ref("");
const activeSpecialFilter = ref(""); // "", "new", "sale"
...
// 👇 Fetch Daftar Tipe Tas (Bag Categories) 👇
const fetchBagCategories = async () => {
  try {
    const res = await axios
      .get(`${BASE_URL}/bag-categories`)
      .catch(() => axios.get(`${BASE_URL}/guest/bag-categories`));
    bagCategories.value = res.data.data || res.data;
  } catch (error) {
    console.error("Gagal menarik daftar tipe tas", error);
  }
};

// 👇 KATEGORISASI (Pemisahan C005 / Accessories) 👇
const normalCategories = computed(() => {
  return categories.value.filter((c) => c.category_code !== "C005" && c.code !== "C005");
});

const accessoriesCategory = computed(() => {
  return categories.value.find((c) => c.category_code === "C005" || c.code === "C005");
});

const isAccessoriesActive = computed(() => {
  const accCat = accessoriesCategory.value;
  return accCat && selectedCategory.value === accCat.category_name;
});

const toggleAccessories = () => {
  const accCat = accessoriesCategory.value;
  if (!accCat) return;
  if (selectedCategory.value === accCat.category_name) {
    selectedCategory.value = "";
  } else {
    selectedCategory.value = accCat.category_name;
  }
};

const setSpecialFilter = (filterParam) => {
  if (activeSpecialFilter.value === filterParam) {
    activeSpecialFilter.value = "";
  } else {
    activeSpecialFilter.value = filterParam;
  }
};
...
// Tangkap Query URL (Dari Mega Menu Header)
const applyRouteQueries = (q) => {
  ...
  if (q.bag_category) selectedBagCategory.value = q.bag_category;
  else selectedBagCategory.value = "";

  if (q.filter === "new-arrivals") activeSpecialFilter.value = "new";
  else if (q.filter === "final-sale") activeSpecialFilter.value = "sale";
  else activeSpecialFilter.value = "";
};

const activeFilterTitle = computed(() => {
  if (activeSpecialFilter.value === "new") return "New Arrivals";
  if (activeSpecialFilter.value === "sale") return "Final Sale";
  if (isAccessoriesActive.value) return "Accessories";
  if (selectedCategory.value) return selectedCategory.value;
  if (selectedBagCategory.value) return selectedBagCategory.value;
...
const filteredProducts = computed(() => {
  ...
  sourceProducts = sourceProducts.filter((p) => {
    // 1. Filter Kategori Koleksi (Category)
    let matchesCategory = true;
    if (selectedCategory.value !== "") {
      const productCategory =
        p.category?.name || p.category_name || p.category?.category_name;
      matchesCategory = productCategory === selectedCategory.value;
    }

    // 2. Filter Tipe Tas (Bag Category)
    let matchesBagCategory = true;
    if (selectedBagCategory.value !== "") {
      const productBagCategory =
        p.bag_category?.name || (p.bagCategory && p.bagCategory.name);
      matchesBagCategory = productBagCategory === selectedBagCategory.value;
    }

    // 3. Filter Spesial (Sale / New Arrivals)
    let matchesSpecial = true;
    if (activeSpecialFilter.value === "sale") {
      matchesSpecial = getDiscountStatus(p).active;
    } else if (activeSpecialFilter.value === "new") {
      matchesSpecial = isNewArrival(p.created_at);
    }

    return matchesCategory && matchesBagCategory && matchesSpecial;
  });

  // 4. Integrasi Fuse.js untuk Pencarian
  if (searchQuery.value.trim() !== "") {
    const fuseOptions = {
      keys: [
        { name: "name", weight: 0.7 },
...
watch([searchQuery, selectedCategory, selectedBagCategory, activeSpecialFilter], () => {
  currentPage.value = 1;
});

// Watch Route Query to sync state when navigating from Header Mega Menu
watch(
  () => route.query,
  (newQ) => {
    applyRouteQueries(newQ);
  },
  { deep: true }
);
...
const handleCategoryChange = () => {
  // Hanya memastikan filter tidak berbenturan
  if (selectedCategory.value !== "" && activeSpecialFilter.value !== "") {
    // Biarkan keduanya aktif agar user bisa filter "Sale" di dalam "Collection X"
  }
};
</script>
