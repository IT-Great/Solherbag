  ...
  >
    ...
    <div v-if="category" class="space-y-10 animate-fade-in">
      ...
      >
        ...
        <div
          v-if="category.bundle_promo?.qty"
          class="bg-blue-50 border border-blue-100 p-6 rounded-xl min-w-[250px] max-w-sm shrink-0"
        >
          <p class="mb-3 text-xs font-bold tracking-widest text-blue-800 uppercase">
            Active Bundle Promo
          </p>
          <div class="space-y-2">
            ...
            <template
              v-if="
                typeof category.bundle_promo.price === 'object' &&
                category.bundle_promo.price !== null
              "
            >
              <div class="flex flex-wrap gap-2 mt-1">
                <div
                  v-for="(val, curr) in category.bundle_promo.price"
                  :key="curr"
                  class="bg-white border border-blue-200 px-2 py-1.5 rounded-lg flex flex-col min-w-[70px] shadow-sm"
                >
                  <span
                  ...
                  <span class="text-sm font-black text-blue-700">
                    {{ getCurrencySymbol(curr) }}{{ formatNumber(val) }}
                  </span>
                </div>
              </div>
            </template>
  ...
  <div class="relative bg-white shadow-sm p-8 border border-gray-100 rounded-2xl min-h-[600px] max-w-6xl mx-auto">
    ...
    <div v-if="isLoading" class="z-20 absolute inset-0 flex justify-center items-center bg-white/60 backdrop-blur-[2px] rounded-2xl transition-all duration-300">
      <div class="flex flex-col items-center">
        ...
        <p class="mt-4 text-xs font-bold tracking-widest text-black uppercase animate-pulse">Loading Details...</p>
      </div>
    </div>

    <div class="flex items-center justify-between pb-6 mb-8 border-b border-gray-100">
      <div class="flex items-center gap-4">
        <button @click="$router.back()" class="p-2 text-gray-400 transition-colors rounded-full hover:text-black hover:bg-gray-50">
        ...
        <div>
          <h1 class="text-2xl font-bold text-gray-800">Category Detail</h1>
          <p class="mt-1 text-xs tracking-widest text-gray-500 uppercase">Overview and related products</p>
        </div>
      </div>
    </div>

    <div v-if="category" class="space-y-10 animate-fade-in">
      <div class="flex flex-col items-start justify-between gap-8 p-6 border border-gray-100 bg-gray-50 md:p-8 rounded-2xl md:flex-row">
        <div class="space-y-4 flex-1">
          <div>
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Category Code</p>
            <p class="font-mono text-lg font-bold text-blue-600">{{ category.code || category.category_code }}</p>
          </div>
          <div>
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Category Name</p>
            <p class="text-2xl font-black tracking-tight text-gray-900 uppercase">{{ category.name || category.category_name }}</p>
          </div>
          <div class="max-w-xl">
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Description</p>
            <p class="text-sm leading-relaxed text-gray-600">{{ category.description || "No description provided for this category." }}</p>
          </div>
        </div>

        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 min-w-[150px] shrink-0 flex flex-col items-center justify-center">
          <p class="mb-2 text-xs font-bold tracking-widest text-gray-400 uppercase">Total Products</p>
        ...
        <div v-if="promoConfig" class="bg-blue-50 border border-blue-100 p-6 rounded-xl min-w-[250px] max-w-sm shrink-0">
          <p class="mb-3 text-xs font-bold tracking-widest text-blue-800 uppercase">Active Promo Engine</p>
          <div class="space-y-2">
            <template v-if="promoConfig.promo_type === 'bundle'">
              <p class="text-sm font-medium text-blue-900">
                Buy <span class="font-black text-xl mx-1">{{ promoConfig.qty }}</span> for
              </p>
              <div class="flex flex-wrap gap-2 mt-1">
                <div v-for="(val, curr) in promoConfig.price" :key="curr" class="bg-white border border-blue-200 px-2 py-1.5 rounded-lg flex flex-col min-w-[70px] shadow-sm">
                  <span class="text-[8px] font-bold text-gray-400 uppercase tracking-widest">{{ curr }}</span>
                  <span class="text-sm font-black text-blue-700">{{ getCurrencySymbol(curr) }}{{ formatNumber(val) }}</span>
                </div>
              </div>
            </template>
            <template v-else-if="promoConfig.promo_type === 'percent'">
              <p class="text-sm font-medium text-blue-900">
                Discount <span class="font-black text-xl mx-1 text-red-600">{{ promoConfig.percent }}%</span>
              </p>
              <p class="text-xs text-blue-800 font-bold mt-1">Min. Buy: {{ formatPrice(promoConfig.min_purchase) }}</p>
              <p v-if="promoConfig.max_discount > 0" class="text-xs text-blue-800 font-bold">Max Cap: {{ formatPrice(promoConfig.max_discount) }}</p>
            </template>

            <div class="pt-2 mt-3 border-t border-blue-200">
              <p v-if="promoConfig.mix_group" class="text-[10px] text-indigo-600 font-mono font-bold mb-1">
                MIX GROUP: {{ promoConfig.mix_group }}
              </p>
              <p class="text-[10px] text-blue-600 font-mono">
                Ends: {{ promoConfig.end_date ? new Date(promoConfig.end_date).toLocaleDateString("en-GB") : 'Lifetime' }}
              </p>
              <span :class="isPromoActive ? 'bg-green-500' : 'bg-red-500'" class="inline-block px-2 py-0.5 mt-1 text-[9px] font-bold text-white uppercase rounded-full">
                {{ isPromoActive ? "Live Now" : "Scheduled/Expired" }}
              </span>
            </div>
          </div>
        </div>
      ...
      <!-- ... (SISA KODE GRID PRODUK TETAP SAMA SEPERTI MILIK ANDA) ... -->
      <div>
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg font-bold tracking-widest text-gray-800 uppercase">Products in this Category</h2>
        </div>
        <div v-if="!category.products || category.products.length === 0" class="py-16 text-center border border-gray-200 border-dashed bg-gray-50 rounded-2xl">
          <p class="font-medium text-gray-500">No products found in this category.</p>
        </div>
        <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          <div v-for="product in category.products" :key="product.id" @click="goToProductDetail(product.id)" class="flex flex-col overflow-hidden transition-all duration-300 bg-white border border-gray-100 cursor-pointer group rounded-xl hover:shadow-lg">
            <div class="relative overflow-hidden bg-gray-100 aspect-square">
              <img v-if="product.image" :src="product.image" class="object-cover w-full h-full transition-transform duration-500 group-hover:scale-105" />
              <div class="absolute flex flex-col gap-1 top-2 left-2">
                <span class="px-2 py-1 rounded text-[8px] font-black uppercase tracking-widest bg-green-500 text-white">{{ product.status }}</span>
              </div>
            </div>
            <div class="flex flex-col flex-grow p-4">
              <p class="text-[9px] font-mono text-gray-400 mb-1">SKU: {{ product.code }}</p>
              <h3 class="mb-2 text-sm font-bold tracking-wide text-gray-900 uppercase line-clamp-2">{{ product.name }}</h3>
              <div class="flex items-end justify-between pt-3 mt-auto border-t border-gray-50">
                <div>
                  ...
                  <span class="font-black text-gray-900 text-sm">{{ formatPrice(product.price) }}</span>
                </div>
              </div>
            </div>
          </div>
...
const goToProductDetail = (productId) => { router.push(`/admin/products/view/${productId}`); };
