        ...
        >
          ...
          <div class="space-y-2">
            <p class="text-sm font-medium text-blue-900">
              Buy
              <span class="mx-1 text-xl font-black">{{ category.bundle_promo.qty }}</span>
              for
            </p>
    ...
    <div v-if="category" class="space-y-10 animate-fade-in">
      ...
      >
        <div class="space-y-6 flex-1">
          ...
          <!-- 👇 [BARU] TAMPILAN LABEL SUBCATEGORIES 👇 -->
          <div
            class="pt-4 border-t border-gray-200"
            v-if="category.subcategories && category.subcategories.length > 0"
          >
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">
              Linked Subcategories ({{ category.subcategories.length }})
            </p>
            <div class="flex flex-wrap gap-2">
              <span
                v-for="(sub, index) in category.subcategories"
                :key="index"
                class="px-3 py-1 text-[10px] font-bold tracking-widest text-gray-800 uppercase bg-white border border-gray-200 rounded-lg shadow-sm"
              >
                {{ sub }}
              </span>
            </div>
          </div>
          <!-- 👆 AKHIR TAMPILAN SUBCATEGORIES 👆 -->
        </div>
            ...
            >
              <div class="flex flex-wrap gap-2 mt-1">
                ...
                >
                  ...
                  >
                  <span class="text-sm font-black text-blue-700"
                    >{{ getCurrencySymbol(curr) }}{{ formatNumber(val) }}</span
                  >
                </div>
              </div>
            </template>
      ...
      <!-- Tabel Produk (Sisa kode tetap sama persis) -->
      <div>
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg font-bold tracking-widest text-gray-800 uppercase">
            Products in this Category
