...
<template>
  <div class="relative max-w-5xl p-8 mx-auto bg-white border border-gray-100 shadow-sm rounded-2xl min-h-[600px]">
    ...
    >
      <div class="flex flex-col items-center">
        ...
        <p class="mt-4 text-xs font-bold tracking-widest text-black uppercase animate-pulse">
          Loading Product...
        </p>
      </div>
    </div>
    ...
    <div v-if="product" class="grid grid-cols-1 gap-12 md:grid-cols-2 animate-fade-in">
      <div class="space-y-4">
        <div class="relative overflow-hidden border border-gray-100 select-none bg-gray-50 rounded-2xl aspect-[4/5] group">
          <img
            v-if="currentMediaType === 'image'"
            :src="currentMediaUrl"
            class="object-cover w-full h-full transition-opacity duration-300"
        ...
        <div v-if="allMedia.length > 1" class="flex gap-3 pb-2 overflow-x-auto custom-scrollbar">
          <div
            v-for="(media, idx) in allMedia"
            :key="idx"
            @click="activeSlide = idx"
            :class="activeSlide === idx ? 'ring-2 ring-black border-transparent' : 'border-gray-200 opacity-60 hover:opacity-100'"
            class="relative w-20 h-24 overflow-hidden transition-all border-2 cursor-pointer shrink-0 rounded-xl bg-gray-50"
          >
            <img v-if="media.type === 'image'" :src="media.url" class="object-cover w-full h-full" />
            <div v-else class="relative flex items-center justify-center w-full h-full text-gray-500 bg-gray-200">
      ...
      <div class="space-y-8">
        <div>
          ...
          <div class="flex items-end gap-4 mt-6">
            <template v-if="product.discount_price">
              <span class="text-3xl font-black text-red-600">{{ formatPrice(product.discount_price) }}</span>
              <span class="mb-1 text-lg text-gray-400 line-through">{{ formatPrice(product.price) }}</span>
            </template>
            <span v-else class="text-3xl font-black text-gray-900">{{ formatPrice(product.price) }}</span>
          </div>
        </div>

        <div class="flex flex-wrap gap-6 p-6 mb-6 border border-gray-100 bg-gray-50 rounded-2xl">
          <div>
          ...
          <div v-if="product.strap_length" class="flex items-start justify-between w-full pt-3 text-sm border-t border-gray-100">
          ...
          <div v-if="parsedProductColors && parsedProductColors.length > 0" class="w-full pt-3 border-t border-gray-100">
            ...
            <div class="flex flex-wrap items-center gap-3">
              <div
                ...
                class="flex items-center gap-1.5 bg-white border border-gray-200 px-2 py-1 rounded-lg shadow-sm"
              >
                <div class="w-3 h-3 border border-gray-300 rounded-full" :style="{ backgroundColor: c.hex }"></div>
                <span class="font-bold text-gray-800 text-[10px] uppercase tracking-wider">{{ c.name }}</span>
              </div>
            </div>
          </div>
        ...
        <div class="flex flex-col gap-5 p-6 mb-6 border border-gray-100 bg-gray-50 rounded-2xl">
          <div class="flex flex-col items-start justify-between gap-4 sm:items-center sm:flex-row">
            <div>
              <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Total Available Stock</p>
            ...
            >
              Manage Batches <span class="transition-transform group-hover:translate-x-1">&rarr;</span>
            </router-link>
          </div>

          <div v-if="product.stocks && product.stocks.length > 0" class="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
            <div class="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-gray-100/50">
              <p class="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Active Stock Batches (FIFO)</p>
              <span class="text-[10px] font-bold text-gray-400 bg-gray-200 px-2 py-0.5 rounded-md">{{ product.stocks.length }} Batch</span>
            </div>

            <div class="p-2 overflow-y-auto max-h-48 custom-scrollbar">
              <div v-for="batch in product.stocks" :key="batch.id" class="flex items-center justify-between p-3 transition-colors border border-transparent rounded-lg hover:bg-gray-50 hover:border-gray-100">
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <p class="font-mono text-sm font-bold text-gray-800">{{ batch.batch_code }}</p>
                    <p class="text-[10px] font-medium text-gray-400 mt-0.5 uppercase tracking-wide">Added: {{ formatDate(batch.created_at) }}</p>
                  </div>
                </div>
                <span class="font-black text-blue-700 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-lg text-xs shadow-sm">{{ batch.quantity }} pcs</span>
              </div>
            </div>
          </div>
          <div v-else class="py-6 text-center bg-white border border-gray-200 border-dashed rounded-xl">
            <p class="text-xs font-medium text-gray-500">No active stock batches found.</p>
          </div>

          <div class="flex items-center justify-between pt-4 mt-2 border-t border-gray-200">
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Data Record</p>
            <div class="text-right">
              <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-0.5">Created At</p>
              <p class="text-xs font-bold text-gray-700">{{ formatDate(product.created_at) }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="border-t border-gray-100 divide-y divide-gray-100">
          <div class="py-4">
            <button @click="toggleSection('description')" class="flex items-center justify-between w-full text-left focus:outline-none">
              <span class="text-sm font-bold tracking-widest text-gray-800 uppercase">Description</span>
              <span class="text-gray-400">{{ activeSection === "description" ? "−" : "+" }}</span>
            </button>
            <div v-show="activeSection === 'description'" class="mt-4 text-sm leading-relaxed text-gray-600 whitespace-pre-wrap">
              {{ product.description || "No description provided." }}
            </div>
          </div>

          <div class="py-4">
            <button @click="toggleSection('design')" class="flex items-center justify-between w-full text-left focus:outline-none">
              <span class="text-sm font-bold tracking-widest text-gray-800 uppercase">Design Details</span>
              <span class="text-gray-400">{{ activeSection === "design" ? "−" : "+" }}</span>
            </button>
            <div v-show="activeSection === 'design'" class="mt-4 text-sm leading-relaxed text-gray-600 whitespace-pre-wrap">
              {{ product.design || "No design details provided." }}
            </div>
          </div>
        </div>
...
const allMedia = computed(() => {
  ...
  if (product.value.variant_images && Array.isArray(product.value.variant_images)) {
    product.value.variant_images.forEach((img) => media.push({ type: "image", url: img }));
  }
  if (product.value.variant_video) media.push({ type: "video", url: product.value.variant_video });
  return media;
});

const currentMediaType = computed(() => allMedia.value[activeSlide.value]?.type || "image");
const currentMediaUrl = computed(() => allMedia.value[activeSlide.value]?.url || "");

const nextSlide = () => activeSlide.value = activeSlide.value === allMedia.value.length - 1 ? 0 : activeSlide.value + 1;
const prevSlide = () => activeSlide.value = activeSlide.value === 0 ? allMedia.value.length - 1 : activeSlide.value - 1;
const toggleSection = (section) => activeSection.value = activeSection.value === section ? null : section;
...
const formatPrice = (value) => new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(value || 0);
const formatDate = (dateString) => {
  if (!dateString) return "-";
