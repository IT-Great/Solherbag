  ...
  <div class="relative bg-white shadow-sm p-8 border border-gray-100 rounded-2xl min-h-[600px]">
          ...
          <tr v-for="i in 5" :key="i" class="border-b border-gray-50">
            <td class="py-6 pl-2 w-[15%]">
              <div class="space-y-2">
                ...
                <div class="h-3 w-20 bg-gray-200 rounded animate-pulse mt-4"></div>
              </div>
            </td>
            <td class="py-6 w-[15%]">
              <div class="flex -space-x-3">
                <div class="h-10 w-10 rounded-full bg-gray-200 border-2 border-white animate-pulse"></div>
                <div class="h-10 w-10 rounded-full bg-gray-200 border-2 border-white animate-pulse"></div>
              </div>
            </td>
            <td class="py-6 w-[20%]">
            ...
            <td class="py-6 w-[10%]">
              <div class="h-5 w-16 mx-auto bg-gray-200 rounded-full animate-pulse"></div>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-else class="overflow-x-auto" id="exportable-table">
      ...
      <table class="w-full text-left border-collapse min-w-[900px]">
        ...
        <tbody v-if="paginatedTransactions.length > 0" class="text-gray-600 text-sm">
          <tr
            v-for="trx in paginatedTransactions"
            :key="trx.id"
            class="group hover:bg-gray-50 border-b border-gray-50 transition cursor-pointer align-top"
          ...
          >
            <td class="py-6 pl-2 w-[15%]">
              <div class="flex flex-col gap-1">
                ...
                <span class="text-gray-400 text-xs">{{ formatDate(trx.created_at) }}</span>
                <div class="mt-2">
                  <span class="block font-bold text-gray-800 text-xs truncate max-w-[150px]" :title="trx.user.first_name + ' ' + trx.user.last_name">
                    {{ trx.user.first_name }} {{ trx.user.last_name }}
                  </span>
                  <span class="block text-gray-400 text-[10px] truncate max-w-[150px]" :title="trx.user.email">
                    {{ trx.user.email }}
                  </span>
                </div>
                <div v-if="trx.status === 'pending' && countdowns[trx.id] !== 'Expired'" class="no-export mt-2 flex items-center gap-1 text-red-600 bg-red-50 w-fit px-2 py-1 rounded-md">
            ...
            <td class="py-6 w-[15%] no-export">
              <div class="flex flex-col gap-2">
                <div class="flex -space-x-3 overflow-hidden py-1 pl-1">
                  <img v-for="(detail, idx) in trx.details.slice(0, 3)" :key="idx" :src="detail.product.image" class="inline-block h-10 w-10 rounded-full border-2 border-white object-cover bg-gray-100 shadow-sm" />
                  <div v-if="trx.details.length > 3" class="flex items-center justify-center h-10 w-10 rounded-full border-2 border-white bg-gray-100 text-[10px] font-bold text-gray-500">
                    +{{ trx.details.length - 3 }}
                  </div>
                </div>
                <span class="text-xs text-gray-500 font-medium">{{ trx.details.length }} Items</span>
            ...
            <td class="py-6 w-[20%]">
              <div class="space-y-4">
                ...
                <div v-if="trx.shipping_method !== 'free'" class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="font-bold text-gray-800 text-[11px] uppercase truncate w-32">{{ trx.courier_company || "Pending" }} - {{ trx.courier_type || "-" }}</p>
                    <p class="text-[10px] text-gray-500 font-mono">Resi: <span class="font-bold text-black">{{ trx.tracking_number || "Waiting..." }}</span></p>
                  </div>
                </div>
    ...
    <div v-if="!isLoading && filteredTransactions.length > 0" class="flex flex-col md:flex-row justify-between items-center gap-4 mt-8 pt-6 border-t border-gray-100">
      <p class="text-sm text-gray-400">
        Showing <span class="font-bold text-black">{{ showingStart }}</span> to <span class="font-bold text-black">{{ showingEnd }}</span> of <span class="font-bold text-black">{{ filteredTransactions.length }}</span> orders
      </p>
...
const getShippingTabCount = (tabValue) => {
  return transactions.value.filter((order) => {
    ...
    if (tabValue === "placed") return ["pending", "placed", "confirmed"].includes(shipStatus);
    if (tabValue === "dropping_off") return ["picked", "dropping_off"].includes(shipStatus);
    if (tabValue === "returning") return ["return_in_transit", "returned"].includes(shipStatus);
    if (tabValue === "issues") return ["cancelled", "rejected", "disposed", "courier_not_found"].includes(shipStatus);
...
const filteredTransactions = computed(() => {
  ...
  return transactions.value.filter((order) => {
    ...
    else if (activeTransactionTab.value === "failed_returned") matchTransaction = ["returned", "shipping_failed"].includes(order.status);
    else matchTransaction = order.status === activeTransactionTab.value;
    ...
    if (activeShippingTab.value === "all") matchShipping = true;
      ...
      if (order.shipping_method === "free") {
        ...
        if (activeShippingTab.value === "placed") matchShipping = ["pending", "placed", "confirmed"].includes(shipStatus);
        else if (activeShippingTab.value === "dropping_off") matchShipping = ["picked", "dropping_off"].includes(shipStatus);
        else if (activeShippingTab.value === "returning") matchShipping = ["return_in_transit", "returned"].includes(shipStatus);
        else if (activeShippingTab.value === "issues") matchShipping = ["cancelled", "rejected", "disposed", "courier_not_found"].includes(shipStatus);
        else matchShipping = shipStatus === activeShippingTab.value;
      }
    }
...
const getGrandTotal = (trx) => parseFloat(trx.total_amount) || 0 + parseFloat(trx.shipping_cost) || 0;
...
const fetchTransactions = async () => {
  ...
  try {
    ...
    transactions.value = filteredData.map((o) => ({ ...o, isCancelling: false }));
    startTimers();
  } catch (error) {
    Swal.fire("Error", "Failed to fetch transactions", "error");
  } finally {
...
<template>
  <div class="relative bg-white shadow-sm p-8 border border-gray-100 rounded-2xl min-h-[600px]">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-center gap-4 mb-8 bg-gray-50/50 p-4 border border-gray-100 rounded-2xl">
      ...
      <div class="relative w-full lg:w-80 shrink-0">
        ...
        <input
          ...
          type="text"
          placeholder="Search Order ID, Name, or Email..."
          class="bg-white pl-10 pr-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-black outline-none w-full text-sm transition shadow-sm"
        />
      </div>

      <div class="flex flex-wrap items-center gap-3 w-full justify-end">
        <div class="flex items-center gap-2">
          <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wide hidden sm:inline">Transaction:</span>
          <select
            v-model="activeTransactionTab"
            class="bg-white px-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-black outline-none text-sm font-semibold cursor-pointer shadow-sm min-w-[140px]"
          >
            <option v-for="tab in transactionTabs" :key="tab.value" :value="tab.value">
              {{ tab.label }}
              <template v-if="tab.value !== 'all'">({{ getTransactionTabCount(tab.value) }})</template>
            </option>
          </select>
        </div>

        <div class="flex items-center gap-2">
          <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wide hidden sm:inline">Shipping:</span>
          <select
            v-model="activeShippingTab"
            class="bg-white px-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-600 outline-none text-sm font-semibold cursor-pointer shadow-sm min-w-[140px]"
          >
            <option v-for="tab in shippingTabs" :key="tab.value" :value="tab.value">
              {{ tab.label }}
              <template v-if="tab.value !== 'all'">({{ getShippingTabCount(tab.value) }})</template>
            </option>
          </select>
        </div>

        <div class="h-6 w-px bg-gray-200 hidden xl:block mx-1"></div>

        <button
          @click="exportToPDF"
          :disabled="paginatedTransactions.length === 0 || isLoading"
          class="flex items-center justify-center gap-2 bg-red-50 hover:bg-red-100 text-red-600 disabled:opacity-50 disabled:cursor-not-allowed px-3 py-2 rounded-xl font-bold text-xs uppercase tracking-widest transition"
          title="Export to PDF"
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        ...
        <button
          @click="exportToExcel"
          :disabled="paginatedTransactions.length === 0 || isLoading"
          class="flex items-center justify-center gap-2 bg-green-50 hover:bg-green-100 text-green-600 disabled:opacity-50 disabled:cursor-not-allowed px-3 py-2 rounded-xl font-bold text-xs uppercase tracking-widest transition"
...
const shippingTabs = [
  ...
  { label: "Placed/Confirmed", value: "placed" }, 
  { label: "Allocated", value: "allocated" },
  { label: "Picking Up", value: "picking_up" },
  { label: "In Transit", value: "dropping_off" },
  { label: "On Hold", value: "on_hold" },
