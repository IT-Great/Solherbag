...
// 👇 [BARU] FUNGSI PRELOAD GAMBAR UNTUK PAGINATION 👇
const preloadImages = (products) => {
  if (!products || products.length === 0) return;
  products.forEach((p) => {
    const mediaArr = getMediaArray(p);
    if (mediaArr && mediaArr.length > 0 && mediaArr[0].type === 'image') {
      const img = new Image();
      img.src = mediaArr[0].url;
    }
  });
};
// 👆 =============================================== 👆
...
const handleCategoryChange = () => {
  pushToRouter({ filter: undefined, sale: undefined });
};
...
const getBundlePromo = (product) => {
  ...
  // Sembunyikan label "Buy X For Y" khusus untuk promo Percent Sale
  if (conf.promo_type === 'percent') return null;

  const priceObj = conf.price || {};
  const finalPrice = priceObj[curr] !== undefined ? priceObj[curr] : (priceObj["IDR"] || 0);

  return { qty: conf.qty || 2, price: finalPrice, curr: curr };
};
...
// 👇 [BARU] WATCHER UNTUK TRIGGER PRELOAD GAMBAR HALAMAN SELANJUTNYA 👇
watch([currentPage, filteredProducts], () => {
  // Hitung produk untuk halaman berikutnya
  const nextStart = currentPage.value * itemsPerPage;
  const nextProducts = filteredProducts.value.slice(nextStart, nextStart + itemsPerPage);
  
  // Beri jeda sedikit agar Vue fokus me-render halaman saat ini dulu,
  // lalu preload gambar halaman berikutnya di background (Eager Cache).
  setTimeout(() => {
