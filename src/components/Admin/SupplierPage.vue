  ...
  >
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
      ...
      <div class="bg-blue-50 p-4 rounded-xl border border-blue-100">
        <p class="text-[10px] text-blue-500 uppercase tracking-widest font-bold">
          Total Invoices
        </p>
        <p class="text-2xl font-black mt-1 text-blue-700">
          {{ stats.total_invoice || 0 }}
        </p>
      </div>
      <div class="bg-red-50 p-4 rounded-xl border border-red-100">
        <p class="text-[10px] text-red-500 uppercase tracking-widest font-bold">
          Unpaid Invoices
        </p>
        <p class="text-2xl font-black mt-1 text-red-700">
          {{ stats.unpaid_invoice || 0 }}
        </p>
      </div>
      <div class="bg-green-50 p-4 rounded-xl border border-green-100">
        <p class="text-[10px] text-green-500 uppercase tracking-widest font-bold">
          Total Value (Rp)
        </p>
        <p class="text-xl font-black mt-1 text-green-700">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          @click="openModal()"
          class="bg-black hover:bg-gray-800 text-white px-4 py-2 rounded-xl text-sm font-bold transition"
        >
          + Add Supplier
        </button>
      </div>
        ...
        <tbody class="text-gray-600 text-sm">
          ...
          >
            ...
            <td class="py-4 text-center space-x-3">
              <button
                @click="openModal(sup)"
                class="text-amber-500 font-bold hover:underline uppercase text-xs"
              >
                Edit
              </button>
              <button
                @click="confirmDelete(sup.id)"
                class="text-red-500 font-bold hover:underline uppercase text-xs"
              >
                Del
              </button>
            </td>
          ...
          <tr v-if="paginatedData.length === 0">
            <td colspan="4" class="text-center py-10 italic text-gray-400">
              No suppliers found.
            </td>
          </tr>
        </tbody>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              ><input
                v-model="form.name"
                type="text"
                class="w-full bg-gray-50 p-3 rounded-xl border outline-none focus:border-black"
                required
              />
            </div>
            <div>
            ...
            <div>
              <label class="text-xs font-bold text-gray-500 uppercase">Acc Number</label
              ><input
            ...
            <div>
              <label class="text-xs font-bold text-gray-500 uppercase">Acc Holder</label
              ><input
                v-model="form.accountnumber_holders_name"
                type="text"
                class="w-full bg-gray-50 p-3 rounded-xl border outline-none focus:border-black"
              />
            </div>
...
const openModal = (data = null) => {
  ...
  form.value = data
    ? { ...data }
    : { name: "", no_telp: "", email: "", bank_name: "", accountnumber: "", address: "" };
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value)
      await axios.put(
        `${BASE_URL}/admin/suppliers/${form.value.id}`,
        form.value,
        axiosConfig
      );
