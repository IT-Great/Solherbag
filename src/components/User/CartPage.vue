            ...
            >
              ...
              >
                <div>
                  ...
                  <!-- 👇 PERBAIKAN: Smart Filter menggunakan shouldShowColor(item) 👇 -->
                  <p
                    v-if="shouldShowColor(item)"
                    class="text-[10px] sm:text-xs text-gray-500 uppercase tracking-widest mt-1.5 flex items-center gap-1.5"
                  >
                    <span
                      class="w-3 h-3 rounded-full border border-gray-300 shadow-inner block"
        ...
        >
          ...
          <div class="flex justify-center gap-6 mt-6 opacity-60 grayscale pb-20 lg:pb-0">
            ...
            >
              ...
              />
              <path
                d="M28.3 10.1H28c-.4 1-.7 1.5-1 3h1.9c-.3-1.5-.3-2.2-.6-3zm2.9 5.9h-1.7c-.1 0-.1 0-.2-.1l-.2-.9-.1-.2h-2.4c-.1 0-.2 0-.2.2l-.3.9c0 .1-.1.1-.1.1h-2.1l.2-.5L27 8.7c0-.5.3-.7.8-.7h1.5c.1 0 .2 0 .2.2l1.4 6.5c.1.4.2.7.2 1.1.1.1.1.1.1.2zm-13.4-.3l.4-1.8c.1 0 .2.1.2.1.7.3 1.4.5 2.1.4.2 0 .5-.1.7-.2.5-.7.1-1.1-.2-.2-.5-.3-.8-.5-.4-.2-.8-.4-1.1-.7-1.2-1-.8-2.4-.1-3.1.6-.4.9-.8 1.7-.8 1.2 0 2.5 0 3.1.2h.1c-.1.6-.2 1.1-.4 1.7-.5-.2-1-.4-1.5-.4-.3 0-.6 0-.9.1-.2 0-.3.1-.4.2-.2.2-.2.5 0 .7l.5.4c.4.2.8.4 1.1.6.5.3 1 .8 1.1 1.4.2.9-.1 1.7-.9 2.3-.5.4-1.1.6-2.1.6-1.1 0-2.5-.2-3.1-.5zm1.5-5.9c-.2 0-.4 0-.5.2l-1.5 6.4c0 .1-.1.2-.2.2h-2.1l1.8-6.5c.1-.2.2-.2.5-.2h2zm-5.1 6.5H12c-.1 0-.2 0-.2-.2L9.4 8.6c0-.2.1-.2.3-.2h2.2c.1 0 .2 0 .2.2l1.1 4c.1.2.2.4.2.5.1-.2.2-.4.3-.6l1.2-3.9c0-.2.1-.2.2-.2h2.1c.1 0 .2 0 .2.2l-3.3 7.5z"
                fill="#1A1F71"
              />
            </svg>
            <svg
...
// 👇 [FUNGSI SMART UI FILTER BARU] 👇
const shouldShowColor = (item) => {
  if (!item.color) return false;
  
  // 1. Ubah ke lowercase & hilangkan spasi untuk dicek
  const cName = parseColorName(item.color).toLowerCase().trim();
  
  // 2. Jika warnanya adalah nilai fallback "basic" atau "default"
  if (cName === 'basic' || cName === 'default' || cName === '-' || cName === '') {
    return false;
  }
  
  // 3. Pengecekan "Dirty Data": Jika color = kata terakhir nama produk
  if (item.product && item.product.name) {
    // Pisahkan nama produk menjadi array kata-kata
    const productWords = item.product.name.toLowerCase().trim().split(/\s+/);
    // Ambil kata paling terakhir
    const lastWord = productWords[productWords.length - 1];
    
    if (cName === lastWord) return false;
  }
