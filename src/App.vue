<!-- <script setup>
// import { useRoute } from "vue-router";
// import Header from "./components/Layout/Header.vue";
// import Footer from "./components/Layout/Footer.vue";

// const route = useRoute();

// import { onMounted } from "vue";
import { watch } from "vue"; // [GANTI: gunakan watch alih-alih onMounted]
import { useRoute } from "vue-router";
import Header from "./components/User/Layout/Header.vue";
import Footer from "./components/User/Layout/Footer.vue";
import Sidebar from "./components/Admin/Layout/Sidebar.vue";
import AdminHeader from "./components/Admin/Layout/AdminHeader.vue";

const route = useRoute();

// onMounted(() => {
//   const urlParams = new URLSearchParams(window.location.search);
//   const refCode = urlParams.get("ref");

//   if (refCode) {
//     // Simpan ke brankas browser. Akan bertahan meskipun user menutup tab.
//     localStorage.setItem("affiliate_ref", refCode);
//   }
// });

// [BARU] Logika pelacak yang 100% responsif terhadap Vue Router
watch(
  () => route.query.ref,
  (newRef) => {
    if (newRef) {
      // Jika mendeteksi ada ?ref= di URL, langsung kunci di brankas browser
      localStorage.setItem("affiliate_ref", newRef);
      console.log("Kode Referal ditangkap:", newRef); // Anda bisa cek di Console nanti
    }
  },
  { immediate: true } // Mengeksekusi pengecekan langsung saat halaman pertama kali dimuat
);
</script>

<template>
  <div class="flex flex-col bg-[#F3F4F6] min-h-screen">
    <Header v-if="!route.meta.hideHeaderFooter" />

    <div v-if="route.meta.isAdmin" class="flex min-h-screen">
      <Sidebar class="flex-shrink-0" />

      <div class="flex flex-col flex-grow min-w-0">
        <AdminHeader />
        <main class="flex-grow bg-gray-100 p-8 overflow-y-auto">
          <router-view />
        </main>
      </div>
    </div>

    <main
      v-else
      class="flex-grow"
      :class="[!route.meta.hideHeaderFooter ? 'pt-20 md:pt-24' : '']"
    >
      <router-view />
    </main>

    <Footer v-if="!route.meta.hideHeaderFooter" />
  </div>
</template>

<style>
/* Reset margin/padding default */
body {
  margin: 0;
  padding: 0;
  font-family: "Inter", sans-serif; /* Pastikan font sesuai keinginan */
}

#app {
  max-width: none !important;
  width: 100%;
}
</style> -->

<!-- <script setup>
import { watch } from "vue"; 
import { useRoute } from "vue-router";
import Header from "./components/User/Layout/Header.vue";
import Footer from "./components/User/Layout/Footer.vue";
import Sidebar from "./components/Admin/Layout/Sidebar.vue";
import AdminHeader from "./components/Admin/Layout/AdminHeader.vue";

const route = useRoute();

// Logika pelacak yang 100% responsif terhadap Vue Router
watch(
  () => route.query.ref,
  (newRef) => {
    if (newRef) {
      localStorage.setItem("affiliate_ref", newRef);
      console.log("Kode Referal ditangkap:", newRef);
    }
  },
  { immediate: true } 
);
</script>

<template>
  <div class="flex flex-col bg-[#F3F4F6] min-h-screen relative">
    <Header v-if="!route.meta.hideHeaderFooter" />

    <div v-if="route.meta.isAdmin" class="flex min-h-screen">
      <Sidebar class="flex-shrink-0" />

      <div class="flex flex-col flex-grow min-w-0">
        <AdminHeader />
        <main class="flex-grow p-8 overflow-y-auto bg-gray-100">
          <router-view />
        </main>
      </div>
    </div>

    <main
      v-else
      class="flex-grow"
      :class="[!route.meta.hideHeaderFooter ? 'pt-20 md:pt-24' : '']"
    >
      <router-view />
    </main>

    <Footer v-if="!route.meta.hideHeaderFooter" />

    <a 
      v-if="!route.meta.isAdmin"
      href="https://wa.me/628883888585" 
      target="_blank" 
      class="fixed z-50 flex items-center justify-center text-white transition-transform duration-300 bg-green-500 rounded-full shadow-2xl w-14 h-14 bottom-6 right-6 hover:scale-110 hover:bg-green-600 group"
    >
      <svg xmlns="http://www.w3.org/2000/svg" class="relative z-10 w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.405-.883-.733-1.48-1.638-1.653-1.935-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 00-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
      </svg>
    </a>
  </div>
</template>

<style>
body {
  margin: 0;
  padding: 0;
  font-family: "Inter", sans-serif; 
}
#app {
  max-width: none !important;
  width: 100%;
}
</style> -->

<script setup>
import { watch, computed } from "vue"; // 👇 [TAMBAHAN] Import computed
import { useRoute } from "vue-router";
import Header from "./components/User/Layout/Header.vue";
import Footer from "./components/User/Layout/Footer.vue";
import Sidebar from "./components/Admin/Layout/Sidebar.vue";
import AdminHeader from "./components/Admin/Layout/AdminHeader.vue";

const route = useRoute();

// Logika pelacak yang 100% responsif terhadap Vue Router
watch(
  () => route.query.ref,
  (newRef) => {
    if (newRef) {
      localStorage.setItem("affiliate_ref", newRef);
      console.log("Kode Referal ditangkap:", newRef);
    }
  },
  { immediate: true } 
);

// 👇 [BARU] Logika Cerdas untuk Menampilkan Tombol WhatsApp 👇
const showWhatsApp = computed(() => {
  // 1. Sembunyikan jika user sedang berada di dasbor Admin
  if (route.meta.isAdmin) return false;
  
  // 2. Sembunyikan jika halaman mensyaratkan sembunyi Header/Footer (Biasanya halaman Login/Register)
  if (route.meta.hideHeaderFooter) return false;

  // 3. Sembunyikan secara eksplisit berdasarkan URL Path (Lapis Keamanan Ganda)
  const hiddenPaths = ['/login', '/admin/login', '/register'];
  if (hiddenPaths.includes(route.path)) return false;

  // Tampilkan di sisa halaman lainnya
  return true;
});
</script>

<template>
  <div class="flex flex-col bg-[#F3F4F6] min-h-screen relative">
    <Header v-if="!route.meta.hideHeaderFooter" />

    <div v-if="route.meta.isAdmin" class="flex min-h-screen">
      <Sidebar class="flex-shrink-0" />

      <div class="flex flex-col flex-grow min-w-0">
        <AdminHeader />
        <main class="flex-grow p-8 overflow-y-auto bg-gray-100">
          <router-view />
        </main>
      </div>
    </div>

    <main
      v-else
      class="flex-grow"
      :class="[!route.meta.hideHeaderFooter ? 'pt-20 md:pt-24' : '']"
    >
      <router-view />
    </main>

    <Footer v-if="!route.meta.hideHeaderFooter" />

    <!-- 👇 FLOATING WHATSAPP BUTTON MENGGUNAKAN COMPUTED PROPERTY 👇 -->
    <a 
      v-if="showWhatsApp"
      href="https://wa.me/628883888585" 
      target="_blank" 
      class="fixed z-50 flex items-center justify-center text-white transition-transform duration-300 bg-green-500 rounded-full shadow-2xl w-14 h-14 bottom-6 right-6 hover:scale-110 hover:bg-green-600 group"
    >
      <svg xmlns="http://www.w3.org/2000/svg" class="relative z-10 w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.405-.883-.733-1.48-1.638-1.653-1.935-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 00-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
      </svg>
    </a>
  </div>
</template>

<style>
body {
  margin: 0;
  padding: 0;
  font-family: "Inter", sans-serif; 
}
#app {
  max-width: none !important;
  width: 100%;
}
</style>
